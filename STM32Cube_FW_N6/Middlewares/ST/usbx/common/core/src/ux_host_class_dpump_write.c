...
ULONG           transfer_request_length;
...
#if defined(UX_HOST_STANDALONE)
    UX_DISABLE
    if (dpump -> ux_host_class_dpump_flags & UX_HOST_CLASS_DPUMP_WRITE_LOCK)
    {
        UX_RESTORE
        return(UX_BUSY);
    }
    dpump -> ux_host_class_dpump_flags |= UX_HOST_CLASS_DPUMP_WRITE_LOCK;
    UX_RESTORE
#else
    ...
    *actual_length =  0;
    ...
    transfer_request =  &dpump -> ux_host_class_dpump_bulk_out_endpoint -> ux_endpoint_transfer_request;
