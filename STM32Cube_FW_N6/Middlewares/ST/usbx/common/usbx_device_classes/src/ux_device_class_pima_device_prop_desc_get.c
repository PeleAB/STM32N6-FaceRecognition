...
/*  DESCRIPTION                                                           */
/*                                                                        */
/*   Return an Device Property Description dataset.                       */
/*                                                                        */
/*                                                                        */
/*  INPUT                                                                 */
/*                                                                        */
...
ULONG                               device_property_desc_dataset_length;
UCHAR                               *device_property_desc_dataset;

    /* If trace is enabled, insert this event into the trace buffer.  */
    UX_TRACE_IN_LINE_INSERT(UX_TRACE_DEVICE_CLASS_PIMA_GET_DEVICE_PROP_DESC_GET, pima, device_property_code, 0, 0, UX_TRACE_DEVICE_CLASS_EVENTS, 0, 0)
    ...
    _ux_utility_short_put(pima_data_buffer + UX_DEVICE_CLASS_PIMA_DATA_HEADER_CODE,
                            UX_DEVICE_CLASS_PIMA_OC_GET_DEVICE_PROP_DESC);
    ...
    /* Ask the application to retrieve for us the device prop description.  */
    device_property_desc_dataset_length = UX_DEVICE_CLASS_PIMA_MAX_PAYLOAD;
    status = pima -> ux_device_class_pima_device_prop_desc_get(pima, device_property_code,
                    &device_property_desc_dataset, &device_property_desc_dataset_length);

    /* Result should always be OK, but to be sure .... */
    if (status != UX_SUCCESS)
    {

        /* We response returned error.  */
    ...
    else
    {
        ...
        _ux_utility_memory_copy(pima_data_buffer + UX_DEVICE_CLASS_PIMA_DATA_HEADER_SIZE,
                        device_property_desc_dataset, device_property_desc_dataset_length); /* Use case of memcpy is verified. */
        ...
        /* Send a data payload with the device prop description dataset.  */
        status =  _ux_device_stack_transfer_request(transfer_request, device_property_desc_dataset_length, 0);
