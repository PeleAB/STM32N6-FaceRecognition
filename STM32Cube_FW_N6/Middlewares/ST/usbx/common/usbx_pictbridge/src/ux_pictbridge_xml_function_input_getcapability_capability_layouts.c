...
UINT    status;
    ...
    if (input_length == 0)
    {
        ...
        pictbridge -> ux_pictbridge_dpsclient.ux_pictbridge_devinfo_layouts_papersize =  0;
    ...
    if (length == input_length)
    {
        ...
        if (_ux_utility_memory_compare(_ux_pictbridge_xml_variable_papersize,input_variable, length) == UX_SUCCESS)
        {
            ...
            if (status == UX_SUCCESS)
            {
                ...
                pictbridge -> ux_pictbridge_dpsclient.ux_pictbridge_devinfo_layouts_papersize =  papersize;
