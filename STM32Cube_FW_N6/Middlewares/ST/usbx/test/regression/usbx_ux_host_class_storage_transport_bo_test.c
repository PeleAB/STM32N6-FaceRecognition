...
static UCHAR                        csw_start_mem_error = 0;
...
static VOID cbw_semaphore_success(struct UX_TEST_ACTION_STRUCT *action, VOID *params)
{

UX_TRANSFER *transfer_request =  &storage -> ux_host_class_storage_bulk_out_endpoint -> ux_endpoint_transfer_request;


    transfer_request->ux_transfer_request_actual_length = 31;
...
static VOID cbw_semaphore_stall(struct UX_TEST_ACTION_STRUCT *action, VOID *params)
{
...
static VOID csw_semaphore_stall(struct UX_TEST_ACTION_STRUCT *action, VOID *params)
{
...
static UX_TEST_HCD_SIM_ACTION stall_bulk_out[] = {
/* function, request to match,
   port action, port status,
   request action, request EP, request data, request actual length, request status,
   status, additional callback,
...
{   UX_HCD_TRANSFER_REQUEST, UX_NULL,
        UX_FALSE, UX_TEST_PORT_STATUS_DISC,
        UX_TEST_MATCH_EP, 0x02, UX_NULL, 0, UX_TRANSFER_STALLED,
        UX_SUCCESS, cbw_semaphore_stall},
{   0   }
};

static UX_TEST_HCD_SIM_ACTION stall_bulk_out2[] = {
/* function, request to match,
   port action, port status,
   request action, request EP, request data, request actual length, request status,
   status, additional callback,
...
static UX_TEST_HCD_SIM_ACTION fail_data_request[] = {
/* function, request to match,
   port action, port status,
   request action, request EP, request data, request actual length, request status,
   status, additional callback,
...
{   UX_HCD_TRANSFER_REQUEST, UX_NULL,
        ...
        UX_SUCCESS, cbw_semaphore_success},
{   UX_HCD_TRANSFER_REQUEST, UX_NULL,
        UX_FALSE, UX_TEST_PORT_STATUS_DISC,
...
static UX_TEST_HCD_SIM_ACTION fail_data_wait[] = {
/* function, request to match,
   port action, port status,
   request action, request EP, request data, request actual length, request status,
   status, additional callback,
...
#define fail_csw_wait fail_data_wait

static UX_TEST_HCD_SIM_ACTION stall_csw_wait2[] = {
/* function, request to match,
   port action, port status,
   request action, request EP, request data, request actual length, request status,
   status, additional callback,
...
{   UX_HCD_TRANSFER_REQUEST, UX_NULL,
        UX_FALSE, UX_TEST_PORT_STATUS_DISC,
        UX_TEST_MATCH_EP, 0x81, UX_NULL, 0, UX_SUCCESS,
        UX_SUCCESS, csw_semaphore_stall},
{   UX_HCD_TRANSFER_REQUEST, UX_NULL,
        UX_FALSE, UX_TEST_PORT_STATUS_DISC,
        UX_TEST_MATCH_EP, 0x81, UX_NULL, 0, UX_SUCCESS,
        UX_SUCCESS, csw_semaphore_stall},
...
static UX_TEST_HCD_SIM_ACTION csw_transfer_fail[] = {
/* function, request to match,
   port action, port status,
   request action, request EP, request data, request actual length, request status,
   status, additional callback,
...
static VOID cbw_replace(struct UX_TEST_ACTION_STRUCT *action, VOID *params)
{
...
static VOID csw_replace(struct UX_TEST_ACTION_STRUCT *action, VOID *params)
{
...
UX_TRANSFER *transfer_request =  &storage -> ux_host_class_storage_bulk_in_endpoint -> ux_endpoint_transfer_request;
    ...
    if (csw_start_mem_error)
        ux_test_utility_sim_mem_alloc_fail_all_start();

    _ux_utility_semaphore_put(&transfer_request -> ux_transfer_request_semaphore);
}

static UX_TEST_HCD_SIM_ACTION replace_cbw_csw[] = {
/* function, request to match,
   port action, port status,
   request action, request EP, request data, request actual length, request status,
   status, additional callback,
...
{   UX_HCD_TRANSFER_REQUEST, UX_NULL,
        ...
        UX_SUCCESS,
        .action_func = csw_replace,
        .no_return = UX_FALSE,
        .do_after = UX_FALSE},
{ 0 }
};

static UX_TEST_HCD_SIM_ACTION replace_cbw_csw_ep0stall[] = {
/* function, request to match,
   port action, port status,
   request action, request EP, request data, request actual length, request status,
   status, additional callback,
...
{   UX_HCD_TRANSFER_REQUEST, UX_NULL,
        UX_FALSE, 0,
        UX_TEST_MATCH_EP, 0x00, UX_NULL, 0, UX_TRANSFER_STALLED,
        UX_TRANSFER_STALLED,
        .action_func = UX_NULL,
        .no_return = UX_FALSE,
        .do_after = UX_FALSE},
...
#else
void    usbx_ux_host_class_storage_transport_bo_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR *                          memory_pointer;
    ...
    /* Inform user.  */
    printf("Running ux_host_class_storage_transport_bo Test..................... ");
    stepinfo("\n");
...
UX_SLAVE_CLASS_STORAGE                      *slave_storage;
    ...
    stepinfo(">>>>>>>>>>>>>>> ux_host_class_storage_transport_bo - CBW STALL retry (2)\n");
    ux_test_hcd_sim_host_set_actions(stall_bulk_out);
    ...
    ux_test_hcd_sim_host_set_actions(stall_bulk_out2);
    status = _ux_host_class_storage_unit_ready_test(storage);
    if (status == UX_SUCCESS)
    {
        printf("ERROR #%d, code 0x%x\n", __LINE__, status);
    ...
    stepinfo(">>>>>>>>>>>>>>> ux_host_class_storage_transport_bo - DATA request error\n");
    ux_test_hcd_sim_host_set_actions(fail_data_request);
    ...
    stepinfo(">>>>>>>>>>>>>>> ux_host_class_storage_transport_bo - DATA timeout\n");
    ux_test_hcd_sim_host_set_actions(fail_data_wait);
    status = _ux_host_class_storage_request_sense(storage);
    if (status == UX_SUCCESS)
    {
        printf("ERROR #%d, code 0x%x\n", __LINE__, status);
    ...
    stepinfo(">>>>>>>>>>>>>>> ux_host_class_storage_transport_bo - CSW timeout\n");
    ux_test_hcd_sim_host_set_actions(fail_csw_wait);
    status = _ux_host_class_storage_unit_ready_test(storage);
    if (status == UX_SUCCESS)
    {
        printf("ERROR #%d, code 0x%x\n", __LINE__, status);
    ...
    stepinfo(">>>>>>>>>>>>>>> ux_host_class_storage_transport_bo - CSW status phase error 1st try\n");
    csw_buffer[UX_HOST_CLASS_STORAGE_CSW_STATUS] = UX_HOST_CLASS_STORAGE_CSW_PHASE_ERROR;
    ux_test_hcd_sim_host_set_actions(replace_cbw_csw);
    status = _ux_host_class_storage_unit_ready_test(storage);
    if (status != UX_SUCCESS)
    {
        printf("ERROR #%d, code 0x%x\n", __LINE__, status);
    ...
    stepinfo(">>>>>>>>>>>>>>> ux_host_class_storage_transport_bo - CSW status residue 1st try\n");
    csw_buffer[UX_HOST_CLASS_STORAGE_CSW_STATUS] = UX_HOST_CLASS_STORAGE_CSW_PASSED;
    csw_buffer[UX_HOST_CLASS_STORAGE_CSW_DATA_RESIDUE] = 0x01;
    ux_test_hcd_sim_host_set_actions(replace_cbw_csw);
    status = _ux_host_class_storage_unit_ready_test(storage);
    if (status != UX_SUCCESS)
    {
    ...
    stepinfo(">>>>>>>>>>>>>>> ux_host_class_storage_transport_bo - RequestSense - UX_MEMORY_INSUFFICIENT\n");
    csw_start_mem_error = 1;
    csw_buffer[UX_HOST_CLASS_STORAGE_CSW_STATUS] = UX_HOST_CLASS_STORAGE_CSW_PASSED;
    csw_buffer[UX_HOST_CLASS_STORAGE_CSW_DATA_RESIDUE] = 0x01;
    ux_test_hcd_sim_host_set_actions(replace_cbw_csw);
    status = _ux_host_class_storage_unit_ready_test(storage);
    ...
    csw_start_mem_error = 0;
    ux_test_utility_sim_mem_alloc_fail_all_stop();

    stepinfo(">>>>>>>>>>>>>>> ux_host_class_storage_transport_bo - RequestSense - GetHalt request error\n");
    csw_buffer[UX_HOST_CLASS_STORAGE_CSW_STATUS] = UX_HOST_CLASS_STORAGE_CSW_PASSED;
    csw_buffer[UX_HOST_CLASS_STORAGE_CSW_DATA_RESIDUE] = 0x01;
    ux_test_hcd_sim_host_set_actions(replace_cbw_csw_ep0stall);
    status = _ux_host_class_storage_unit_ready_test(storage);
    if (status == UX_SUCCESS)
    {
        printf("ERROR #%d, code 0x%x\n", __LINE__, status);
    ...
    // stepinfo(">>>>>>>>>>>>>>> ux_host_class_storage_transport_bo - RequestSense - ClearHalt request error\n");

    stepinfo(">>>>>>>>>>>>>>> ux_host_class_storage_transport_bo - CSW stall twice\n");
    ux_test_hcd_sim_host_set_actions(stall_csw_wait2);
    status = _ux_host_class_storage_unit_ready_test(storage);
    if (status == UX_SUCCESS)
    {
        printf("ERROR #%d, code 0x%x\n", __LINE__, status);
    ...
    stepinfo(">>>>>>>>>>>>>>> ux_host_class_storage_transport_bo - CSW transfer fails\n");
    ux_test_hcd_sim_host_set_actions(csw_transfer_fail);
    status = _ux_host_class_storage_unit_ready_test(storage);
    if (status == UX_SUCCESS)
    {
        printf("ERROR #%d, code 0x%x\n", __LINE__, status);
