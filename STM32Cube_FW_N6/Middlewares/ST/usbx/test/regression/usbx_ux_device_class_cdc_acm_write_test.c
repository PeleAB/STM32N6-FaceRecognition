...
#else
void usbx_ux_device_class_cdc_acm_write_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR *                  memory_pointer;
    ...
    printf("Running ux_device_class_cdc_acm_write Test.......................... ");
    stepinfo("\n");
...
UINT                                                i;
    ...
    /* Test write. */
    stepinfo(">>>>>>>>>>>>>>>> Test write disconnect after mutex_on\n");

    test_thread_action = 1;
    tx_thread_resume(&tx_test_thread_simulation_1);

    for (i = 0; i < 10; i ++)
    {
        if (test_thread_action < 2)
            _ux_utility_delay_ms(10);
    }
    if (test_thread_action < 2)
    {
        printf("ERROR %d: test no progress\n", __LINE__);
        error_counter ++;
    }
...
ULONG                                               actual_length;
    ...
    while(1)
    {
        if (test_thread_action == 1)
        {
            ux_test_set_main_action_list_from_array(disconnect_after_mutex_on);
            status = ux_device_class_cdc_acm_write(cdc_acm_slave, buffer, 128, &actual_length);
            if (status == UX_SUCCESS)
            {
