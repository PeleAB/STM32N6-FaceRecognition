...
#else
void    usbx_ux_device_stack_set_feature_test_application_define(void *first_unused_memory)
#endif
{
...
UX_SLAVE_CLASS_HID_PARAMETER    hid_parameter;
    ...
    /* Inform user.  */
    printf("Running ux_device_stack_set_feature Test............................ ");
...
UX_SLAVE_TRANSFER        *slave_transfer_request;
    ...
    /* Test HNP requests */
    stepinfo(">>>>>>>>>>>>>>>> Test SetFeature(UX_OTG_FEATURE_A_HNP_SUPPORT)\n");
    ...
    if (status != UX_SUCCESS)
    {
        printf("ERROR #%d: SetFeature(UX_OTG_FEATURE_A_HNP_SUPPORT) fail\n", __LINE__);
        error_counter ++;
    }
    if ((_ux_system_otg -> ux_system_otg_slave_set_feature_flag & UX_OTG_FEATURE_A_HNP_SUPPORT) == 0)
    {
        printf("ERROR #%d: SetFeature(UX_OTG_FEATURE_A_HNP_SUPPORT) not changing status\n", __LINE__);
        error_counter ++;
    }
#else
    ...
    if (status == UX_SUCCESS)
    {
        printf("ERROR #%d: SetFeature(UX_OTG_FEATURE_A_HNP_SUPPORT) must fail\n", __LINE__);
        error_counter ++;
    }
#endif

    /* Test HNP requests */
    stepinfo(">>>>>>>>>>>>>>>> Test SetFeature(UX_OTG_FEATURE_B_HNP_ENABLE)\n");
    transfer_request -> ux_transfer_request_data_pointer =      buffer;
    transfer_request -> ux_transfer_request_requested_length =  0;
    transfer_request -> ux_transfer_request_function =          UX_SET_FEATURE;
    transfer_request -> ux_transfer_request_type =              UX_REQUEST_OUT | UX_REQUEST_TYPE_STANDARD | UX_REQUEST_TARGET_DEVICE;
    ...
    if (status != UX_SUCCESS)
    {
        ...
        printf("ERROR #%d: SetFeature(UX_OTG_FEATURE_B_HNP_ENABLE) fail\n", __LINE__);
        error_counter ++;
    }
    if ((_ux_system_otg -> ux_system_otg_slave_set_feature_flag & UX_OTG_FEATURE_B_HNP_ENABLE) == 0)
    {

        printf("ERROR #%d: SetFeature(UX_OTG_FEATURE_B_HNP_ENABLE) not changing status\n", __LINE__);
        error_counter ++;
    }
#else
    expect_errors = UX_TRUE;
    ...
    if (status == UX_SUCCESS)
    {
        printf("ERROR #%d: SetFeature(UX_OTG_FEATURE_B_HNP_ENABLE) must fail\n", __LINE__);
        error_counter ++;
    }
#endif

    /* Test HNP requests */
    stepinfo(">>>>>>>>>>>>>>>> Test SetFeature(UX_OTG_FEATURE_A_ALT_HNP_SUPPORT)\n");
    transfer_request -> ux_transfer_request_data_pointer =      buffer;
    transfer_request -> ux_transfer_request_requested_length =  0;
    transfer_request -> ux_transfer_request_function =          UX_SET_FEATURE;
    transfer_request -> ux_transfer_request_type =              UX_REQUEST_OUT | UX_REQUEST_TYPE_STANDARD | UX_REQUEST_TARGET_DEVICE;
    transfer_request -> ux_transfer_request_value =             UX_OTG_FEATURE_A_ALT_HNP_SUPPORT;
    transfer_request -> ux_transfer_request_index =             0;
#if 0 /* defined(UX_OTG_SUPPORT) */ /* No support yet.  */
    status = ux_host_stack_transfer_request(transfer_request);
    if (status != UX_SUCCESS)
    {

        printf("ERROR #%d: SetFeature(UX_OTG_FEATURE_A_ALT_HNP_SUPPORT) fail\n", __LINE__);
        error_counter ++;
    }
#else
    expect_errors = UX_TRUE;
    ...
    if (status == UX_SUCCESS)
    {
        printf("ERROR #%d: SetFeature(UX_OTG_FEATURE_A_ALT_HNP_SUPPORT) must fail\n", __LINE__);
        error_counter ++;
    }
#endif
