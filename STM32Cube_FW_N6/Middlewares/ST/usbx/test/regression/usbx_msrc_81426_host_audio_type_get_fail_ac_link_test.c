...
#else
void usbx_msrc_81426_host_audio_type_get_fail_ac_link_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR *                  memory_pointer;
    ...
    /* Inform user.  */
    printf("Running MSRC 81426 - Audio Host Fail AC AS Link Test................ ");
...
static void _audio_type_get_fail_ac_tests(void)
{
ULONG mem_free;
    ...
    ux_test_dcd_sim_slave_disconnect();
    ux_test_hcd_sim_host_disconnect();
    UX_TEST_ASSERT(host_audio_ac == UX_NULL && host_audio_rx == UX_NULL && host_audio_tx == UX_NULL);

    /* Set framework.  */
    _ux_system_slave -> ux_system_slave_device_framework_full_speed = device_framework_modified;

    /* Prepare descriptor ASD.  */
    /* ------------------------ Audio 2.0 AS Interface Descriptor *//* @189+9=198 */
    /* 0  bLength, bDescriptorType, bDescriptorSubtype            */// 16,  0x24, 0x01,
    /* 3  bTerminalLink, bmControls                               */// 0x03,0x00,
    _ux_utility_memory_copy(device_framework_modified, device_framework_full_speed, DEVICE_FRAMEWORK_LENGTH_FULL_SPEED);
    UX_TEST_ASSERT(device_framework_modified[FRAMEWORK_POS_FULL_SPEED_ASD + 0] == 16);
    UX_TEST_ASSERT(device_framework_modified[FRAMEWORK_POS_FULL_SPEED_ASD + 1] == 0x24);
    UX_TEST_ASSERT(device_framework_modified[FRAMEWORK_POS_FULL_SPEED_ASD + 3] == 0x03);/* bTerminalLink  */

    /* Log memory.  */
    mem_free = ux_test_regular_memory_free();

    stepinfo(">>>>>>>>>> Fake descriptor _type_get fail - ASD::bTerminalLink=0\n");
    device_framework_modified[FRAMEWORK_POS_FULL_SPEED_ASD + 3] = 0;
    /* Connect. */
    error_callback_counter = 0;
    ux_test_dcd_sim_slave_connect(UX_FULL_SPEED_DEVICE);
    ux_test_hcd_sim_host_connect(UX_FULL_SPEED_DEVICE);
    ...
    UX_TEST_ASSERT(host_audio_ac->ux_host_class_audio_as[0] == host_audio_tx);
    UX_TEST_ASSERT(host_audio_ac->ux_host_class_audio_as[1] == UX_NULL);
#if defined(UX_HOST_CLASS_AUDIO_INTERRUPT_SUPPORT)
    UX_TEST_ASSERT(host_audio_tx->ux_host_class_audio_ac == host_audio_ac);
    UX_TEST_ASSERT(host_audio_tx->ux_host_class_audio_ac_as == 0);
#endif
    ...
    device_framework_modified[FRAMEWORK_POS_FULL_SPEED_ASD + 3] = 0x03;
