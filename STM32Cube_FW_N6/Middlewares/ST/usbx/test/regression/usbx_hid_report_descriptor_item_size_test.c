...
static UCHAR hid_report_descriptor[] = {
    ...
    0x15, 0xab,                    //   LOGICAL_MINIMUM () - One byte
    0x26, 0xab, 0xcd,              //   LOGICAL_MAXIMUM () - Two bytes
    0x35, 0x00,                    //   PHYSICAL_MINIMUM ()
    0x47, 0xab, 0xcd, 0xef, 0x01,  //   PHYSICAL_MAXIMUM () - Four bytes
...
#else
void    usbx_hid_report_descriptor_item_size_test_application_define(void *first_unused_memory)
#endif
{
...
UINT                            descriptor_size = HID_REPORT_LENGTH;
    ...
    /* Inform user.  */
    printf("Running HID Report Descriptor Item Size Test........................ ");
...
UX_HOST_CLASS_HID_FIELD             *report_field;
    ...
    if(
       report_field -> ux_host_class_hid_field_logical_min != 0xab ||
       report_field -> ux_host_class_hid_field_logical_max != 0xcdab ||
       report_field -> ux_host_class_hid_field_physical_max != 0x01efcdab
       )
    {
        printf("Error on line %d\n", __LINE__);
        test_control_return(1);
