...
#else
void usbx_msrc_81024_host_cdc_ecm_rx_length_less_than_14_test_application_define(void *first_unused_memory)
#endif
{

    /* Inform user.  */
    printf("Running MSRC 81024 - Host CDC ECM RX length less than 14 Test....... ");
...
static void msrc_81024_test(void)
{
NX_PACKET_POOL  *packet_pool;
NX_PACKET       *packet;
UINT            i;

    packet_pool = cdc_ecm_device->ux_slave_class_cdc_ecm_packet_pool;
    ...
    for (i = 0; i < 100; i ++)
    {
        ...
        *packet->nx_packet_append_ptr ++= 0; packet->nx_packet_length = 1;
        UX_TEST_CHECK_SUCCESS(_ux_device_class_cdc_ecm_write(cdc_ecm_device, packet));
        tx_thread_sleep(1);
    }
}
...
static void post_init_device()
{
    ...
    /* Test MSRC 81024, packet should be discarded service continue.  */
    msrc_81024_test();
