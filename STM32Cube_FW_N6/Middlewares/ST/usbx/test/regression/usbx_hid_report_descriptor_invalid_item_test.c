...
static UCHAR hid_report_descriptor[] = {
    ...
    0x81, 0xab,                    //   INPUT (Data,Var,Abs)
    0xc0                           // END_COLLECTION
};
#define HID_REPORT_LENGTH sizeof(hid_report_descriptor)/sizeof(hid_report_descriptor[0])

static UCHAR hid_report_descriptor_type_error[] = {

    0x05, 0x01,                    // USAGE_PAGE (Generic Desktop)
    0x09, 0x06,                    // USAGE (Keyboard)
    0xa1, 0x01,                    // COLLECTION (Application)
    /*------- 0x75 -> 0x7d generate tag type error                            */
    0x7d, 0x01,                    //   REPORT_SIZE (1)
    0x95, 0x08,                    //   REPORT_COUNT (8)
    0x81, 0xab,                    //   INPUT (Data,Var,Abs)
    0xc0                           // END_COLLECTION
};
static UCHAR hid_report_descriptor_length_error[] = {
    ...
    0x75, 0x01,                    //   REPORT_SIZE (1)
    0x95, 0x08,                    //   REPORT_COUNT (8)
    /*------- 0x81 -> 0x83 generate tag length error                            */
    0x83, 0xab,                    //   INPUT (Data,Var,Abs)
    0xc0                           // END_COLLECTION
};
...
static UCHAR device_framework_full_speed[DEVICE_FRAMEWORK_LENGTH_FULL_SPEED] = {
    ...
    /* HID descriptor, modify descriptor length <> actual length. */
...
#else
void    usbx_hid_report_descriptor_invalid_item_test_application_define(void *first_unused_memory)
#endif
{
...
UINT                            descriptor_size = HID_REPORT_LENGTH;
    ...
    /* Inform user.  */
    printf("Running HID Report Descriptor Invalid Item Type/Length Test......... ");
...
UINT status;
    ...
    _ux_utility_memory_copy(hid_report_descriptor, hid_report_descriptor_type_error, sizeof(hid_report_descriptor));
    ...
    ux_test_hcd_sim_host_disconnect();
    _ux_utility_memory_copy(hid_report_descriptor, hid_report_descriptor_length_error, sizeof(hid_report_descriptor));
    ux_test_hcd_sim_host_connect(UX_FULL_SPEED_DEVICE);
    status = demo_class_hid_get();
    if (status == UX_SUCCESS)
    {
