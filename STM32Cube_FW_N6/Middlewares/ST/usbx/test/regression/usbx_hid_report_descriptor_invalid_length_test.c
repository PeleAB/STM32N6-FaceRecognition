...
static UCHAR device_framework_full_speed[DEVICE_FRAMEWORK_LENGTH_FULL_SPEED] = {
    ...
    /* HID descriptor, modify descriptor length <> actual length. */
        0x09, 0x21, 0x10, 0x01, 0x21, 0x01, 0x22, LSB(HID_REPORT_LENGTH+8),
        MSB(HID_REPORT_LENGTH+8),
...
#else
void    usbx_hid_report_descriptor_invalid_length_test_application_define(void *first_unused_memory)
#endif
{
...
UINT                            descriptor_size = HID_REPORT_LENGTH;
    ...
    /* Inform user.  */
    printf("Running HID Report Descriptor Invalid Descriptor Length Test........ ");
