...
#else
void    usbx_ux_device_class_storage_uninitialize_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR *                          memory_pointer;
    ...
    /* Inform user.  */
    printf("Running ux_device_class_storage_uninitialize Test................... ");
    stepinfo("\n");
...
UX_SLAVE_CLASS_COMMAND                      slave_command;
    ...
    /* Select a class that not used and confirm its instance is NULL.  */
    slave_class = &_ux_system_slave -> ux_system_slave_class_array[1];
    slave_class->ux_slave_class_instance = UX_NULL;

    slave_command.ux_slave_class_command_class_ptr = slave_class;

    status = _ux_device_class_storage_uninitialize(&slave_command);
    if (status != UX_SUCCESS)
    {
