...
#else
void usbx_ux_host_class_cdc_acm_capabilities_get_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR *                  memory_pointer;
    ...
    printf("Running ux_host_class_cdc_acm_capabilities_get Test................. ");
...
UINT                                                status;
    ...
    /* Test descriptor length error. */
    stepinfo(">>>>>>>>>>>>>>>> Test capabilities_get (length error)\n");

    /* Request error on 1st try. */
    ux_test_hcd_sim_host_set_actions(error_on_GetCfgDescr0);
    ...
    /* Request length mismatch on 1st try. */
    ux_test_hcd_sim_host_set_actions(length_error_on_GetCfgDescr0);
    status = _ux_host_class_cdc_acm_capabilities_get(cdc_acm_host_control);
    if (status != UX_SUCCESS)
    {
    ...
    /* Request length mismatch on 2nd try. */
    ux_test_hcd_sim_host_set_actions(length_error_on_GetCfgDescr1);
    status = _ux_host_class_cdc_acm_capabilities_get(cdc_acm_host_control);
    if (status != UX_SUCCESS)
    {
    ...
    /* Request descriptor corrupt. */

    /* Modify interface descriptor type, expect error. */
    device_framework_full_speed[18 + 9 + 8 + 1] = 0;
    status = _ux_host_class_cdc_acm_capabilities_get(cdc_acm_host_control);
    if (status != UX_SUCCESS)
    {
    ...
    /* Restore configuration descriptor length. */
    device_framework_full_speed[18 + 9 + 8 + 1] = UX_INTERFACE_DESCRIPTOR_ITEM;

    /* Request descriptor subclass is UX_HOST_CLASS_CDC_DLC_SUBCLASS. */

    /* Modify interface descriptor type, expect no error. */
    device_framework_full_speed[18 + 9 + 8 + 6] = UX_HOST_CLASS_CDC_DLC_SUBCLASS;
    status = _ux_host_class_cdc_acm_capabilities_get(cdc_acm_host_control);
    if (status != UX_SUCCESS)
    {
    ...
    /* Restore configuration descriptor length. */
    device_framework_full_speed[18 + 9 + 8 + 6] = UX_HOST_CLASS_CDC_ACM_SUBCLASS;

    /* Request descriptor subclass is not expected. */

    /* Modify interface descriptor type, expect no error. */
    device_framework_full_speed[18 + 9 + 8 + 6] = 0xEE;
    status = _ux_host_class_cdc_acm_capabilities_get(cdc_acm_host_control);
    if (status != UX_SUCCESS)
    {
