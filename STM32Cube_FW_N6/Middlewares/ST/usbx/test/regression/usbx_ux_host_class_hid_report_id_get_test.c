...
#else
void    usbx_ux_host_class_hid_report_id_get_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR *                          memory_pointer;
    ...
    /* Inform user.  */
    printf("Running ux_host_class_hid_report_id_get Test........................ ");
...
UINT                                status;
UX_HOST_CLASS_HID_REPORT_GET_ID     report_get_id = { 0 };
...
UX_HOST_CLASS_HID_KEYBOARD          *keyboard;
    ...
    status = _ux_host_class_hid_report_id_get(UX_NULL, &report_get_id);
    ...
    status = _ux_host_class_hid_report_id_get(hid, &report_get_id);
    ...
    /**************************************************/
    /** Test case: switch (report_id -> ux_host_class_hid_report_get_type) feature case. **/
    /** Why direct: can't force internal APIs to use feature report type. **/
    /**************************************************/

    /* Set the report type to unknown value. */
    report_get_id.ux_host_class_hid_report_get_type = UX_HOST_CLASS_HID_REPORT_TYPE_FEATURE;

    status = _ux_host_class_hid_report_id_get(hid, &report_get_id);

    /* There ain't any feature reports. */
    if(status == UX_SUCCESS)
    {
    ...
    /**************************************************/
    /** Test case: switch (report_id -> ux_host_class_hid_report_get_type) default case. **/
    /** Why direct: can't force internal APIs to use incorrect report type. **/
    /**************************************************/

    /* Set the report type to unknown value. */
    report_get_id.ux_host_class_hid_report_get_type = 0;
    ...
    /**************************************************/
    /** Test case: read reports until there is no more. **/
    /**************************************************/
    ...
    /**************************************************/
    /** Test case: if (hid_report -> ux_host_class_hid_report_next_report == UX_NULL). **/
    /** Why direct: in order to fail, must be called multiple times, but only called once in activate functions (i.e. ux_host_class_hid_keyboard_activate()). **/
    /**************************************************/

    /* Set the report type to input and the current report to the only report (this will cause the API to get the next report, which doesn't exist). */
    report_get_id.ux_host_class_hid_report_get_type = UX_HOST_CLASS_HID_REPORT_TYPE_INPUT;
