...
#else
void    usbx_ux_host_stack_hcd_register_test_application_define(void *first_unused_memory)
#endif
{
...
UX_SLAVE_CLASS_HID_PARAMETER    hid_parameter;
    ...
    /* Inform user.  */
    printf("Running ux_host_stack_hcd_register Test............................. ");
    stepinfo("\n");
...
UCHAR                           invalid_class_name[UX_MAX_CLASS_NAME_LENGTH+2];
    ...
    stepinfo(">>>>>>>>> _ux_host_stack_hcd_register - error if class_name exceed UX_MAX_CLASS_NAME_LENGTH\n");
    _ux_utility_memory_set(invalid_class_name, 'a', sizeof(invalid_class_name));
    _ux_utility_memory_copy(invalid_class_name, _ux_system_host_hcd_simulator_name, 16);
    status =  _ux_host_stack_hcd_register(invalid_class_name, _ux_test_hcd_sim_host_initialize, 0, 0);
    if (status != UX_ERROR)
    {
