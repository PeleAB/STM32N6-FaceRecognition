...
static VOID UX_DCD_CHANGE_STATE_is_called(UX_TEST_ACTION *action, VOID *_params)
{
    call_counter ++;
}

static UX_TEST_DCD_SIM_ACTION monitor_UX_DCD_CHANGE_STATE[] = {
/* function, request to match,
   port action, port status,
   request action, request EP, request data, request actual length, request status,
   status, additional callback,
   no_return */
{   UX_DCD_CHANGE_STATE, NULL,
        UX_FALSE, 0,
        0, 0, UX_NULL, 0, 0,
        UX_SUCCESS, UX_DCD_CHANGE_STATE_is_called},
{   0   }
};
...
static ULONG test_notification_type = 0;
static ULONG test_notification_value = 0;
static VOID  test_cdc_acm_device_status_change_callback(struct UX_HOST_CLASS_CDC_ACM_STRUCT *cdc_acm,
                                                 ULONG  notification_type, ULONG notification_value)
{

    test_notification_type = notification_type;
...
#else
void usbx_ux_host_class_cdc_acm_transfer_request_completed_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR *                  memory_pointer;
    ...
    printf("Running ux_host_class_cdc_acm_transfer_request_completed Test....... ");
...
UX_SLAVE_INTERFACE                                  *slave_interface;
UX_SLAVE_ENDPOINT                                   *slave_endpoint;
UX_SLAVE_ENDPOINT                                   *slave_interrupt_endpoint = UX_NULL;
UX_SLAVE_TRANSFER                                   *slave_transfer;
UCHAR                                               *backup_pointer;
    ...
    slave_interface = slave_device->ux_slave_device_first_interface;
    while(slave_interface)
    {
        slave_endpoint = slave_interface->ux_slave_interface_first_endpoint;
        while(slave_endpoint)
        {
            if ((slave_endpoint->ux_slave_endpoint_descriptor.bmAttributes & UX_MASK_ENDPOINT_TYPE) == UX_INTERRUPT_ENDPOINT)
            {
                slave_interrupt_endpoint = slave_endpoint;
                break;
            }
            slave_endpoint = slave_endpoint->ux_slave_endpoint_next_endpoint;
        }
        slave_interface = slave_interface->ux_slave_interface_next_interface;
    }
    if (!slave_interrupt_endpoint)
    {

        printf("ERROR #%d: no interrupt endpoint\n", __LINE__);
        test_control_return(1);
    }

    stepinfo(">>>>>>>>>>>>>>>> Send notification and check callback\n");

    test_notification_type = 0;
    test_notification_value = 0;

    /* Send interrupt request.  */
    buffer[UX_HOST_CLASS_CDC_ACM_NPF_REQUEST_TYPE]      = 0xA1;
    buffer[UX_HOST_CLASS_CDC_ACM_NPF_NOTIFICATION_TYPE] = 0x40;
    buffer[UX_HOST_CLASS_CDC_ACM_NPF_VALUE] =             0x03;
    buffer[UX_HOST_CLASS_CDC_ACM_NPF_VALUE + 1] =         0x04;
    buffer[UX_HOST_CLASS_CDC_ACM_NPF_INDEX] =             0x00;
    buffer[UX_HOST_CLASS_CDC_ACM_NPF_INDEX + 1] =         0x00;

    /* Prepare transfer request.  */
    slave_transfer = &slave_interrupt_endpoint->ux_slave_endpoint_transfer_request;
    backup_pointer = slave_transfer->ux_slave_transfer_request_data_pointer;
    slave_transfer->ux_slave_transfer_request_data_pointer = buffer;

    status = _ux_device_stack_transfer_request(slave_transfer, 8, 8);
    if (status != UX_SUCCESS)
    {
        printf("ERROR #%d: transfer error %x\n", __LINE__, status);
        test_control_return(1);
    }

    /* Wait a while for transfer to complete.  */
    _ux_utility_delay_ms(30);

    if (test_notification_type != 0x00)
    {
        printf("ERROR #%d: type 0x%lx not correct\n", __LINE__, test_notification_type);
        error_counter ++;
    }

    if (test_notification_value != 0x0000)
    {
        printf("ERROR #%d: value 0x%lx not correct\n", __LINE__, test_notification_value);
        error_counter ++;
    }

    /* Enable callback.  */
    ux_host_class_cdc_acm_ioctl(cdc_acm_host_control, UX_HOST_CLASS_CDC_ACM_IOCTL_NOTIFICATION_CALLBACK, (VOID *)test_cdc_acm_device_status_change_callback);
    ...
    if (test_notification_type != 0x40)
    {
        printf("ERROR #%d: type 0x%lx not correct\n", __LINE__, test_notification_type);
        error_counter ++;
    }

    if (test_notification_value != 0x0403)
    {
        printf("ERROR #%d: value 0x%lx not correct\n", __LINE__, test_notification_value);
        error_counter ++;
    }

    /* Reset transfer request (avoid buffer free error).  */
    slave_transfer->ux_slave_transfer_request_data_pointer = backup_pointer;
