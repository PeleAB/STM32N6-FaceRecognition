...
#else
void    usbx_ux_host_class_hid_interrupt_endpoint_search_int_out_application_define(void *first_unused_memory)
#endif
{
...
CHAR *                          memory_pointer;
    ...
    /* Inform user.  */
    printf("Running ux_host_class_hid_interrupt_endpoint_search Interrupt OUT Test .......... ");
...
UINT status;
    ...
    /* Check if we have found the interrupt OUT endpoint.  */
    if (hid -> ux_host_class_hid_interrupt_out_endpoint == UX_NULL)
    {

        printf("Error on line %d, error code: %d\n", __LINE__, UX_ENDPOINT_HANDLE_UNKNOWN);
