...
#else
void    usbx_ux_host_class_storage_media_read_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR *                          memory_pointer;
    ...
    /* Inform user.  */
    printf("Running ux_host_class_storage_media_read Test....................... ");
    stepinfo("\n");
...
UX_INTERFACE                                *interface;
    ...
    stepinfo(">>>>>>>>>>>>>>> ux_host_class_storage_media_read - Data cut off\n");
    ux_test_hcd_sim_host_set_actions(replace_cbw_data_csw);
    status = _ux_host_class_storage_media_read(storage, 0, 1, buffer);
    if (status != UX_ERROR)
    {
        printf("ERROR #%d, code 0x%x\n", __LINE__, status);
        test_control_return(1);
