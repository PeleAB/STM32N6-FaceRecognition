...
#else
void    usbx_ux_utility_semaphore_test_application_define(void *first_unused_memory)
#endif
{
...
UINT status;
    ...
    /* Inform user.  */
    printf("Running ux_utility_semaphore... Test................................ ");
...
TX_SEMAPHORE test_semaphore;
    ...
    status = _ux_utility_semaphore_create(&test_semaphore, "test semaphore", 1);
    if (status != UX_SUCCESS)
    {

        printf("ERROR #%d: sem_create 0x%x\n", __LINE__, status);
        error_counter ++;
    }
    ...
    if (status == UX_SUCCESS)
    {
        ...
        printf("ERROR #%d: sem_create must fail\n", __LINE__);
        error_counter ++;
    }

    status = _ux_utility_semaphore_get(&test_semaphore, 100);
    if (status != UX_SUCCESS)
    {

        printf("ERROR #%d: sem_get 0x%x\n", __LINE__, status);
        error_counter ++;
    }

    status = _ux_utility_semaphore_get(&test_semaphore, 10);
    if (status == UX_SUCCESS)
    {
