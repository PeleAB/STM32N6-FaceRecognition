...
{
UX_HOST_CLASS_HID_CLIENT    *client_inst = (UX_HOST_CLASS_HID_CLIENT*)inst;

    stepinfo("hChg:%lx, %p, %p\n", event, cls, inst);
    switch(event)
    {
        ...
        case UX_HID_CLIENT_REMOVAL:
            UX_TEST_ASSERT(!ux_test_memory_is_freed(client_inst -> ux_host_class_hid_client_local_instance));
            if (hid_client == client_inst)
            {
                hid_client = UX_NULL;
            }
            break;
...
CHAR *                          memory_pointer;
    ...
    /* Inform user.  */
    printf("Running MSRC 81326 - Host HID RemoteControl Free and Callback Test.. ");
