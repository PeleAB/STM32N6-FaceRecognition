...
#else
void    usbx_hid_report_descriptor_multiple_reports_input_test_application_define(void *first_unused_memory)
#endif
{
...
UINT                            descriptor_size = HID_REPORT_LENGTH;
    ...
    /* Inform user.  */
    printf("Running HID Report Descriptor Multiple Reports Input Test........... ");
...
UX_HOST_CLASS_HID_REPORT            *input_report_descriptor;
    ...
    report_buffer_compressed[0] = 0x01;
    report_buffer_compressed[1] = 0x23;

    /* Request decompression. */
    status = _ux_host_class_hid_report_decompress(hid, &input_report_request, report_buffer_compressed, 2);
    ...
    if (
        ...
        report_buffer_decompressed[3] != 0x00 ||
        report_buffer_decompressed[4] != 0x00010003 ||
        report_buffer_decompressed[5] != 0x03 ||
        report_buffer_decompressed[6] != 0x00010004 ||
        report_buffer_decompressed[7] != 0x02
        )
    {
    ...
    report_buffer_compressed[0] = 0x02; // First byte is the Report ID.
    report_buffer_compressed[1] = 0x45;
    report_buffer_compressed[2] = 0x67;
    report_buffer_compressed[3] = 0x89;

    /* Request decompression. */
    status = _ux_host_class_hid_report_decompress(hid, &input_report_request, report_buffer_compressed, 3);
    ...
    if (
        report_buffer_decompressed[0] != 0x00010005 ||
        report_buffer_decompressed[1] != 0x45 ||
        report_buffer_decompressed[2] != 0x00010006 ||
        report_buffer_decompressed[3] != 0x67 ||
        report_buffer_decompressed[4] != 0x00010007 ||
        report_buffer_decompressed[5] != 0x89
        )
    {
    ...
    report_buffer_compressed[0] = 0x04; // First byte is the Report ID.
    report_buffer_compressed[1] = 0xab;
    report_buffer_compressed[2] = 0xcd;
    ...
    if (
        report_buffer_decompressed[0] != 0x00010008 ||
        report_buffer_decompressed[1] != 0xcdab
        )
    {
