...
#else
void usbx_uxe_host_gser_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR                            *memory_pointer;
    ...
    /* Inform user.  */
    printf("Running uxe_host_gser APIs Test..................................... ");
...
{
UINT                                            status;
UX_HOST_CLASS_GSER                              dummy_gser_inst;
UX_HOST_CLASS_GSER                              *dummy_gser = &dummy_gser_inst;
UX_HOST_CLASS_GSER_RECEPTION                    dummy_reception;
UCHAR                                           dummy_buffer[64];
ULONG                                           dummy_ul;

    /* ux_host_class_gser_read()  */
    status = ux_host_class_gser_read(UX_NULL, 0, dummy_buffer, 8, &dummy_ul);
    UX_TEST_CHECK_CODE(UX_INVALID_PARAMETER, status);
    status = ux_host_class_gser_read(dummy_gser, 0, UX_NULL, 8, &dummy_ul);
    UX_TEST_CHECK_CODE(UX_INVALID_PARAMETER, status);
    status = ux_host_class_gser_read(dummy_gser, 0, dummy_buffer, 8, UX_NULL);
    UX_TEST_CHECK_CODE(UX_INVALID_PARAMETER, status);

    /* ux_host_class_gser_write()  */
    status = ux_host_class_gser_write(UX_NULL, 0, dummy_buffer, 8, &dummy_ul);
    UX_TEST_CHECK_CODE(UX_INVALID_PARAMETER, status);
    status = ux_host_class_gser_write(dummy_gser, 0, UX_NULL, 8, &dummy_ul);
    UX_TEST_CHECK_CODE(UX_INVALID_PARAMETER, status);
    status = ux_host_class_gser_write(dummy_gser, 0, dummy_buffer, 8, UX_NULL);
    UX_TEST_CHECK_CODE(UX_INVALID_PARAMETER, status);

    /* ux_host_class_gser_ioctl()  */
    status = ux_host_class_gser_ioctl(UX_NULL, 0, 0, UX_NULL);
    UX_TEST_CHECK_CODE(UX_INVALID_PARAMETER, status);

    /* ux_host_class_gser_reception_start()  */
    status = ux_host_class_gser_reception_start(UX_NULL, &dummy_reception);
    UX_TEST_CHECK_CODE(UX_INVALID_PARAMETER, status);
    status = ux_host_class_gser_reception_start(dummy_gser, UX_NULL);
    UX_TEST_CHECK_CODE(UX_INVALID_PARAMETER, status);

    /* ux_host_class_gser_reception_stop()  */
    status = ux_host_class_gser_reception_stop(UX_NULL, &dummy_reception);
    UX_TEST_CHECK_CODE(UX_INVALID_PARAMETER, status);
    status = ux_host_class_gser_reception_stop(dummy_gser, UX_NULL);
    UX_TEST_CHECK_CODE(UX_INVALID_PARAMETER, status);
