...
#define DEVICE_FRAMEWORK_LENGTH_FULL_SPEED 114
static UCHAR device_framework_full_speed[] = {
    ...
    /* Configuration descriptor */
        0x09, 0x02, 0x20, 0x00, 0x01, 0x02, 0x00, 0x80,
        0xFA,
    ...
    /* Configuration descriptor */
        0x09, 0x02, 0x20, 0x00, 0x01, 0x03, 0x00, 0xC0,
        0xFA,
...
static VOID error_callback(UINT system_level, UINT system_context, UINT error_code)
{
    if ((error_code != UX_CONFIGURATION_HANDLE_UNKNOWN) &&
        (error_code != UX_OVER_CURRENT_CONDITION))
    {
        /* Failed test.  */
...
#else
void    usbx_ux_host_stack_device_configuration_reset_select_test_application_define(void *first_unused_memory)
#endif
{
...
UX_SLAVE_CLASS_DPUMP_PARAMETER  parameter;
    ...
    /* Inform user.  */
    printf("Running ux_host_stack_device_configuration_reset/select Test........ ");
...
UINT                        status;
UX_HOST_CLASS               *class;
static UX_CONFIGURATION     test_configuration;
UX_CONFIGURATION            *configuration1;
UX_CONFIGURATION            *configuration2;
UX_CONFIGURATION            *configuration3;
UX_DEVICE                   *device;
    ...
    /* At this point, the data pump class has been found.  Now call _ux_host_stack_device_configuration_select
       with invalid configuration.  */
    status = ux_host_stack_device_configuration_select(&test_configuration);
    ...
    configuration1 = dpump -> ux_host_class_dpump_interface -> ux_interface_configuration;
    configuration2 = configuration1 -> ux_configuration_next_configuration;
    configuration3 = configuration2 -> ux_configuration_next_configuration;

    /* Select configuration 2.  */
    status = ux_host_stack_device_configuration_select(configuration2);
    if (status != UX_SUCCESS)
    {
    ...
    status = ux_host_stack_device_configuration_select(configuration1);
    if (status != UX_SUCCESS)
    {
...
#if UX_MAX_DEVICES > 1
    /* Set max power to test over current condition.  */
    device -> ux_device_max_power = 50;

    /* Select configuration 2 again.  */
    status = ux_host_stack_device_configuration_select(configuration2);
    if (status != UX_OVER_CURRENT_CONDITION)
    {
    ...
    status = ux_host_stack_device_configuration_select(configuration3);
    if (status != UX_SUCCESS)
    {
    ...
    /* Set max power back.  */
    device -> ux_device_max_power = 250;

    /* Select configuration 2 for configuration_reset test.  */
    status = ux_host_stack_device_configuration_select(configuration2);
    if (status != UX_SUCCESS)
    {
