...
#else
void    usbx_ux_host_class_hid_remote_control_entry_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR *                          memory_pointer;
    ...
    /* Inform user.  */
    printf("Running ux_host_class_hid_remote_control_entry Test................. ");
...
UX_HOST_CLASS_HID_CLIENT_COMMAND    client_command;
    ...
    /* Set to unknown command. */
    client_command.ux_host_class_hid_client_command_request = 0xdeadbeef;

    status = _ux_host_class_hid_remote_control_entry(&client_command);
    if (status != UX_ERROR)
    {
