...
#else
void    usbx_ux_utility_pci_write_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR                            *memory_pointer;
    ...
    /* Inform user.  */
    printf("Running ux_utility_pci_write Test .................................. ");
...
ULONG       addr, value;
    ...
    _ux_utility_pci_write(0, 0, 0, 0, 1, 0);
    ...
    _ux_utility_pci_write(0, 0, 0, 0, 2, 8);
    _ux_utility_pci_write(0, 0, 0, 0, 3, 16);
    _ux_utility_pci_write(0, 0, 0, 0, 4, 32);
    ux_test_sim_outp_log_get(1, &addr, &value);
    if (value != 2)
    {
        printf("ERROR #%d: expected %x but got %lx\n", __LINE__, 2, value);
        error_counter ++;
    }
    ux_test_sim_outp_log_get(3, &addr, &value);
    if (value != 3)
    {
        printf("ERROR #%d: expected %x but got %lx\n", __LINE__, 3, value);
        error_counter ++;
    }
    ux_test_sim_outp_log_get(5, &addr, &value);
    if (value != 4)
    {
        printf("ERROR #%d: expected %x but got %lx\n", __LINE__, 4, value);
        error_counter ++;
