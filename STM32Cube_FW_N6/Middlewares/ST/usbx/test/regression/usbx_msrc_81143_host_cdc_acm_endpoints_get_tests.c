...
#define _CFG_TOTAL_LEN (9+8 +9+5+4+5+5+7+ 9+7)
...
UX_HOST_CLASS_CDC_ACM *cdc_acm_inst = (UX_HOST_CLASS_CDC_ACM *) inst;
    ...
    case UX_DEVICE_INSERTION:
        ...
        if (cdc_acm_inst->ux_host_class_cdc_acm_interface->ux_interface_descriptor.bInterfaceClass==0x02)
            break;
...
#else
void    usbx_msrc_81143_host_cdc_acm_endpoints_get_tests_application_define(void *first_unused_memory)
#endif
{
...
CHAR *                  memory_pointer;
    ...
    printf("Running MSRC 81143 - Host CDC ACM EPs Get Test...................... ");
    ...
    status =  ux_host_stack_class_register(_ux_system_host_class_cdc_acm_name, _ux_host_class_cdc_acm_entry);
...
{
    if (device_dummy && host_cdc_acm)
        return(UX_SUCCESS);
    if (error_callback_counter >= 7)
        return(UX_SUCCESS);
    return(UX_ERROR);
}
...
{
    if (device_dummy == UX_NULL && host_cdc_acm == UX_NULL)
        return(UX_SUCCESS);
    return(UX_ERROR);
}
...
UX_INTERFACE                                        *interface;
    ...
    while(interface)
    {
        if (interface != device -> ux_device_current_configuration -> ux_configuration_first_interface)
