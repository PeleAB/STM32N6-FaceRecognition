...
#else
void    usbx_ux_device_stack_transfer_request_test_application_define(void *first_unused_memory)
#endif
{
...
UX_SLAVE_CLASS_HID_PARAMETER    hid_parameter;
    ...
    /* Inform user.  */
    printf("Running ux_device_stack_transfer_request Test....................... ");
...
UX_SLAVE_TRANSFER        *slave_transfer_request;
    ...
    /* Get slave device and transfer request.  */
    slave_device = &_ux_system_slave->ux_system_slave_device;
    slave_transfer_request = &slave_device->ux_slave_device_control_endpoint.ux_slave_endpoint_transfer_request;

    /* Reset configuration.  */
    status = ux_host_stack_device_configuration_reset(device);
    if (status != UX_SUCCESS)
    {
        printf("ERROR #%d: Configuration Reset fail\n", __LINE__);
        test_control_return(1);
    }
    /* Confirm it's in addressed state.
       See USBX-188, USBX-161
     */
    if (slave_device->ux_slave_device_state != UX_DEVICE_ADDRESSED)
    {
        // printf("Correct state: %x -> %x (UX_DEVICE_ADDRESSED)\n", slave_device->ux_slave_device_state, UX_DEVICE_ADDRESSED);
        slave_device->ux_slave_device_state = UX_DEVICE_ADDRESSED;
    }
    ...
    if (slave_device->ux_slave_device_state != UX_DEVICE_RESET)
    {
        printf("ERROR #%d: slave state not UX_DEVICE_RESET but 0x%lx\n", __LINE__, slave_device->ux_slave_device_state);
        test_control_return(1);
    }

    status = ux_device_stack_transfer_request(slave_transfer_request, 8, 8);
    if (status == UX_SUCCESS)
    {

        printf("ERROR #%d: ux_device_stack_transfer_request(8, 8) should fail\n", __LINE__);
        error_counter ++;
    }
