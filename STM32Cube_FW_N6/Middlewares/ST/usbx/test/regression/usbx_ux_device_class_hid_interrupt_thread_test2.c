...
#else
void    usbx_ux_device_class_hid_interrupt_thread_test2_application_define(void *first_unused_memory)
#endif
{
...
CHAR *                          memory_pointer;
    ...
    /* Inform user.  */
    printf("Running ux_device_class_hid_interrupt_thread Test 2................. ");
...
ULONG                           tmp_timeout;
    ...
    /**************************************************/
    /** Test case: _ux_device_stack_transfer_request return with different value in case of no event. **/
    /**************************************************/
    /* Add an event to let the interrupt thread run. */
    *(ULONG *)hid_event.ux_device_class_hid_event_buffer = 0xdeadbeef;
    hid_event.ux_device_class_hid_event_length = 4;
    hid_event.ux_device_class_hid_event_report_id = 0;
    tmp_timeout = device_hid->ux_device_class_hid_event_wait_timeout;
    /* Set the timeout to 2s and sleep 10s later to generate >=3 times of no event cases */
    device_hid->ux_device_class_hid_event_wait_timeout = 2;
    ...
    /* restore ux_device_class_hid_event_wait_timeout */
    device_hid->ux_device_class_hid_event_wait_timeout = tmp_timeout;
    /* Restore state for next test. */
    _ux_system_slave -> ux_system_slave_dcd.ux_slave_dcd_function = _ux_dcd_sim_slave_function;

    /**************************************************/
    /** Test case: status =  _ux_utility_event_flags_get(&hid -> ux_device_class_hid_event_flags_group, UX_DEVICE_CLASS_HID_NEW_EVENT,
                                                                                            TX_OR_CLEAR, &actual_flags, TX_WAIT_FOREVER); fails. **/
    /**************************************************/
    status = ux_utility_event_flags_delete(&device_hid -> ux_device_class_hid_event_flags_group);
    if (status != UX_SUCCESS)
    {
