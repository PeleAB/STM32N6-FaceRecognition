...
#else
void    usbx_ux_host_class_hid_descriptor_parse_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR *                          memory_pointer;
    ...
    /* Inform user.  */
    printf("Running ux_host_class_hid_descriptor_parse Test..................... ");
...
static UINT ux_hcd_sim_host_entry_filter_descriptor_too_small_test(UX_HCD *hcd, UINT function, VOID *parameter)
{
...
UX_TRANSFER *transfer_request;
    ...
    case UX_HCD_TRANSFER_REQUEST:
        ...
        /* Set the total configuration length to be greater than 0. */
        transfer_request -> ux_transfer_request_data_pointer[2] = 0x01;

        /* Set the first descriptor length to be less than 3. */
        transfer_request -> ux_transfer_request_data_pointer[0] = 0x00;
...
static UINT ux_hcd_sim_host_entry_filter_descriptor_too_large_test(UX_HCD *hcd, UINT function, VOID *parameter)
{
...
UX_TRANSFER *transfer_request;
    ...
    case UX_HCD_TRANSFER_REQUEST:
        ...
        /* Set the first descriptor length to be greater than the total configuration length and at least 3. */
        transfer_request -> ux_transfer_request_data_pointer[0] = 0x03;
...
// UX_MEMORY_BLOCK                    *original_regular_memory_block = _ux_system -> ux_system_memory_byte_pool[UX_MEMORY_BYTE_POOL_REGULAR] -> ux_byte_pool_start;
// UX_MEMORY_BLOCK                    *original_cache_safe_memory_block = _ux_system -> ux_system_cache_safe_memory_pool_start;
...
#if 0 /* Covered in basic memory tests. */
    /**************************************************/
    /** Test case: descriptor =  _ux_utility_memory_allocate(UX_SAFE_ALIGN, UX_CACHE_SAFE_MEMORY, UX_CONFIGURATION_DESCRIPTOR_LENGTH); fails **/
    /**************************************************/
...
#endif
    /**************************************************/
    /** Test case: ux_host_stack_transfer_request() failed. **/
    /** Why direct: To fail normally, I'd have to replace the entry function and somehow distinguish GET_DESCRIPTOR requests. **/
    /**************************************************/

    /* Replace the entry function. */
    ...
    /**************************************************/
    /** Test case: if (descriptor_length < 3) **/
    /** Why direct: To fail normally, I'd have to replace the entry function and somehow distinguish GET_DESCRIPTOR requests
              OR pass a malformed descriptor, but then the device stack would've errored out before. **/
    /**************************************************/

    /* Replace the entry function. */
    hcd = UX_DEVICE_HCD_GET(hid -> ux_host_class_hid_device);
    tmp = (ALIGN_TYPE)hcd -> ux_hcd_entry_function;
    hcd -> ux_hcd_entry_function = ux_hcd_sim_host_entry_filter_descriptor_too_small_test;
    ...
    /**************************************************/
    /** Test case: descriptor length larger than total configuration descriptor length. **/
    /** Why direct: To fail normally, I'd have to replace the entry function and somehow distinguish GET_DESCRIPTOR requests
              OR pass a malformed descriptor, but then the device stack would've errored out before. **/
    /**************************************************/
    ...
    hcd = UX_DEVICE_HCD_GET(hid -> ux_host_class_hid_device);
    tmp = (ALIGN_TYPE)hcd -> ux_hcd_entry_function;
    hcd -> ux_hcd_entry_function = ux_hcd_sim_host_entry_filter_descriptor_too_large_test;
