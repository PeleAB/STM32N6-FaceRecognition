...
#else
void    usbx_ux_utility_string_length_check_test_application_define(void *first_unused_memory)
#endif
{
...
const CHAR                       expect_error[] = {
    ...
    /* Inform user.  */
    printf("Running ux_utility_string_length_check Test......................... ");
...
UINT string_length;
    ...
    stepinfo(">>>>>>>>> _ux_utility_string_length_check - error if string is NULL\n");
    status = _ux_utility_string_length_check(UX_NULL, &string_length, 128);
    if (status != UX_ERROR)
    {
        printf("ERROR #%d: expect UX_ERROR\n", __LINE__);
        error_counter ++;
    }

    stepinfo(">>>>>>>>> _ux_utility_string_length_check - no error if string_length_ptr is NULL\n");
    status = _ux_utility_string_length_check(_ux_system_host_class_dpump_name, UX_NULL, 128);
    if (status != UX_SUCCESS)
    {
        printf("ERROR #%d: expect UX_SUCCESS\n", __LINE__);
        error_counter ++;
    }
