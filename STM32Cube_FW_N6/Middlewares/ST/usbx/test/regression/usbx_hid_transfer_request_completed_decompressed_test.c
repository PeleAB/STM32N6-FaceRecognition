...
#else
void    usbx_hid_transfer_request_completed_decompressed_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR *                          memory_pointer;
    ...
    /* Inform user.  */
    printf("Running HID Transfer Request Completed Decompressed Test............ ");
...
VOID  dummy_transfer_request_callback(UX_HOST_CLASS_HID_REPORT_CALLBACK *callback)
{

static INT  expected_button_value = -1;
static INT  expected_x_value = -1;
static INT  expected_y_value = -1;
static INT  expected_mouse_wheel_value = -1;
INT         actual_button_value;
INT         actual_x_value;
INT         actual_y_value;
INT         actual_mouse_wheel_value;
ULONG      *buffer;


    buffer = callback->ux_host_class_hid_report_callback_buffer;
    ...
    actual_x_value = buffer[9];
    actual_y_value = buffer[11];
    actual_mouse_wheel_value = buffer[13];

    /* If uninitialized, initialize expected_value. */
    if (expected_button_value == -1)
    {

        expected_button_value =         actual_button_value;
        expected_x_value =              actual_x_value;
        expected_y_value =              actual_y_value;
        expected_mouse_wheel_value =    actual_mouse_wheel_value;
    }

    if (actual_button_value != expected_button_value ||
        actual_x_value != expected_x_value ||
        actual_y_value != expected_y_value ||
        actual_mouse_wheel_value != expected_mouse_wheel_value)
    {
    ...
    expected_button_value = (0x7 & (expected_button_value + 1));
    expected_x_value++;
    expected_y_value++;
    expected_mouse_wheel_value++;
...
UCHAR                               buffer[1024];
    ...
    callback.ux_host_class_hid_report_callback_id =         0;
    callback.ux_host_class_hid_report_callback_function =   dummy_transfer_request_callback;
    callback.ux_host_class_hid_report_callback_buffer =     buffer;
    callback.ux_host_class_hid_report_callback_flags =      UX_HOST_CLASS_HID_REPORT_DECOMPRESSED;
    callback.ux_host_class_hid_report_callback_length =     sizeof(buffer);

    status = ux_host_class_hid_report_callback_register(hid, &callback);
    if (status != UX_SUCCESS)
    {
    ...
    /* Wait until callback has been invoked at least 16 times. */
    while(num_callbacks < 16)
        tx_thread_sleep(10);
...
UX_SLAVE_CLASS_HID_EVENT        hid_event;
    ...
    hid_event.ux_device_class_hid_event_buffer[2] = 3;

    /* Set initial value for mouse wheel. */
    hid_event.ux_device_class_hid_event_buffer[3] = 4;
    ...
    while (1)
    {
        ...
        while (device -> ux_slave_device_state == UX_DEVICE_CONFIGURED)
        {
            ...
            hid_event.ux_device_class_hid_event_buffer[3]++;
        }
    }
}
