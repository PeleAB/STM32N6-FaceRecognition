...
#else
void    usbx_hid_report_descriptor_previous_report_test_application_define(void *first_unused_memory)
#endif
{
...
UINT                            descriptor_size = HID_REPORT_LENGTH;
    ...
    /* Inform user.  */
    printf("Running HID Report Descriptor Previous Report Test....................... ");
...
UX_HOST_CLASS_HID_FIELD             *report_field;
    ...
    /* Check the first field in the report. */
    report_field = report_id.ux_host_class_hid_report_get_report -> ux_host_class_hid_report_field;
    if(
       report_field -> ux_host_class_hid_field_report_id != 0x01 ||
       report_field -> ux_host_class_hid_field_report_count != 0x04 ||
       report_field -> ux_host_class_hid_field_report_size != 0x04
       )
    {
    ...
    /* Is there not another field? (There should be!) */
    if (report_field -> ux_host_class_hid_field_next_field == UX_NULL)
    {
    ...
    if(
       report_field -> ux_host_class_hid_field_report_id != 0x01 ||
       report_field -> ux_host_class_hid_field_report_count != 0x01 ||
       report_field -> ux_host_class_hid_field_report_size != 0x10
       )
    {
