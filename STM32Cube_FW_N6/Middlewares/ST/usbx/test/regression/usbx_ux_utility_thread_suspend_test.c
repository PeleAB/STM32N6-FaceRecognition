...
#else
void    usbx_ux_utility_thread_suspend_test_application_define(void *first_unused_memory)
#endif
{
...
UINT status;
    ...
    /* Inform user.  */
    printf("Running ux_utility_thread_suspend Test.............................. ");
...
UINT status;
    ...
    /* Suspend the thread.  */
    status = _ux_utility_thread_suspend(UX_NULL);
    if (status != TX_THREAD_ERROR)
    {
        printf("ERROR #%d: expect TX_THREAD_ERROR but got 0x%x\n", __LINE__, status);
        error_counter ++;
    }

    /* Suspend the thread.  */
    status = _ux_utility_thread_suspend(&ux_test_thread_simulation_1);
    if (status != TX_THREAD_ERROR)
    {
        printf("ERROR #%d: expect TX_THREAD_ERROR but got 0x%x\n", __LINE__, status);
        error_counter ++;
    ...
    if (status != UX_SUCCESS)
    {
        printf("ERROR #%d: expect OK but got 0x%x\n", __LINE__, status);
        error_counter ++;
    }
