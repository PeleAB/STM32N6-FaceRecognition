...
struct mode_sense6_parameter_header {

    /* @0 The MODE DATA LENGTH field indicates the length in bytes of the following
        data that is available to be transferred. The mode data length does not
        include the number of bytes in the MODE DATA LENGTH field.  */
    UCHAR mode_data_length;
    UCHAR medium_type;

    /* @2 0x80 for write protect.  */
    UCHAR wp_buffered_mode_speed;
    UCHAR block_descriptor_length;
};
static struct mode_sense6_parameter_header mode_sense_parameter6 = {
    .mode_data_length = 3,
    .medium_type = 0x00,
    .wp_buffered_mode_speed = 0x00,
    .block_descriptor_length = 0,
};

struct mode_sense10_parameter_header {
    USHORT mode_data_length;
    UCHAR  medium_type;

    /* @3 0x80 for write protect.  */
    UCHAR  wp_buffered_mode_speed;
    UCHAR  reserved[2];
    USHORT block_descriptor_length;
};
static struct mode_sense10_parameter_header mode_sense_parameter10 = {
    .mode_data_length = 6,
    .medium_type = 0x00,
    .wp_buffered_mode_speed = 0x00,
    .reserved = {0, 0},
...
UCHAR       cmd  = *(cbw + UX_HOST_CLASS_STORAGE_CBW_CB + UX_HOST_CLASS_STORAGE_MODE_SENSE_OPERATION);


    if (cmd == UX_HOST_CLASS_STORAGE_SCSI_MODE_SENSE_SHORT)
    {
        _ux_utility_memory_copy(transfer_request->ux_transfer_request_data_pointer, &mode_sense_parameter6, sizeof(mode_sense_parameter6));
        transfer_request->ux_transfer_request_actual_length = sizeof(mode_sense_parameter6);
    }
    else
    {
        _ux_utility_memory_copy(transfer_request->ux_transfer_request_data_pointer, &mode_sense_parameter10, sizeof(mode_sense_parameter10));
        transfer_request->ux_transfer_request_actual_length = sizeof(mode_sense_parameter10);
    }
    transfer_request->ux_transfer_request_completion_code = UX_SUCCESS;
    _ux_utility_semaphore_put(&transfer_request -> ux_transfer_request_semaphore);
}
...
#else
void    usbx_ux_host_class_storage_media_protection_check_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR *                          memory_pointer;
    ...
    /* Inform user.  */
    printf("Running ux_host_class_storage_media_protection_check Test........... ");
    stepinfo("\n");
...
ULONG                                       rfree, cfree;
    ...
    stepinfo(">>>>>>>>>>>>>>> ux_host_class_storage_media_protection_check - UX_SUCCESS\n");
    status = _ux_host_class_storage_media_protection_check(storage);
    if (status != UX_SUCCESS)
    {
        printf("ERROR #%d, code 0x%x\n", __LINE__, status);
        test_control_return(1);
    ...
    {
        printf("ERROR #%d: expect unprotected\n", __LINE__);
        test_control_return(1);
    }

    stepinfo(">>>>>>>>>>>>>>> ux_host_class_storage_media_protection_check - WP\n");
    ux_test_hcd_sim_host_set_actions(replace_cbw_data_csw);
    status = _ux_host_class_storage_media_protection_check(storage);
    if (status != UX_SUCCESS)
    {
        printf("ERROR #%d, code 0x%x\n", __LINE__, status);
    ...
    mode_sense_parameter6.wp_buffered_mode_speed = 0x80;
    ux_test_hcd_sim_host_set_actions(replace_cbw_data_csw);
    status = _ux_host_class_storage_media_protection_check(storage);
    if (status != UX_SUCCESS)
    {
    ...
    if (storage -> ux_host_class_storage_write_protected_media == UX_FALSE)
    {
        printf("ERROR #%d: expect protected\n", __LINE__);
        test_control_return(1);
    }

    stepinfo(">>>>>>>>>>>>>>> ux_host_class_storage_media_protection_check - RBC\n");
    ux_test_hcd_sim_host_set_actions(replace_cbw_data_csw);
    storage -> ux_host_class_storage_interface -> ux_interface_descriptor.bInterfaceSubClass = UX_HOST_CLASS_STORAGE_SUBCLASS_RBC;
    status = _ux_host_class_storage_media_protection_check(storage);
    if (status != UX_SUCCESS)
    {
        printf("ERROR #%d, code 0x%x\n", __LINE__, status);
    ...
    stepinfo(">>>>>>>>>>>>>>> ux_host_class_storage_media_protection_check - UX_MEMORY_INSUFFICIENT\n");
    ...
    if (rfree != _ux_system -> ux_system_memory_byte_pool[UX_MEMORY_BYTE_POOL_REGULAR] -> ux_byte_pool_available ||
        cfree != _ux_system -> ux_system_memory_byte_pool[UX_MEMORY_BYTE_POOL_CACHE_SAFE] -> ux_byte_pool_available)
    {
        printf("ERROR #%d, memory leap!\n", __LINE__);
        test_control_return(1);
    }

    stepinfo(">>>>>>>>>>>>>>> ux_host_class_storage_media_protection_check - transfer error\n");
