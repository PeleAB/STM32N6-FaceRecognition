...
#else
void    usbx_ux_host_class_storage_configure_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR *                          memory_pointer;
    ...
    /* Inform user.  */
    printf("Running ux_host_class_storage_configure Test........................ ");
    stepinfo("\n");
...
UX_DEVICE                                   parent_device;
    ...
    stepinfo(">>>>>>>>>>>>>>> ux_host_stack_device_configuration_reset\n");
    ...
    stepinfo(">>>>>>>>>>>>>>> ux_host_class_storage_configure - SUCCESS\n");
    ...
    stepinfo(">>>>>>>>>>>>>>> ux_host_class_storage_configure - configuration_get FAIL\n");
    status = _ux_host_stack_device_configuration_reset(device);
    if (status != UX_SUCCESS)
    {
        printf("ERROR #%d, code 0x%x\n", __LINE__, status);
...
#if UX_MAX_DEVICES > 1
    stepinfo(">>>>>>>>>>>>>>> ux_host_class_storage_configure - BUS_POWERED parent and me\n");
    ...
    stepinfo(">>>>>>>>>>>>>>> ux_host_class_storage_configure - configuration_select FAIL\n");
    ...
    stepinfo(">>>>>>>>>>>>>>> ux_host_class_storage_configure - interface_get FAIL\n");
    interface = configuration -> ux_configuration_first_interface;
    configuration -> ux_configuration_first_interface = UX_NULL;
