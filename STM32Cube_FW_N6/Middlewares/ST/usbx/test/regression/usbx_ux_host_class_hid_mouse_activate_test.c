...
static UX_TEST_HCD_SIM_ACTION stall_on_transfer_0[] = {
...
{   UX_HCD_TRANSFER_REQUEST, UX_NULL,
        UX_FALSE, 0,
        UX_TEST_SIM_REQ_ANSWER, 0, UX_NULL, 0, UX_TRANSFER_STALLED,
        UX_TRANSFER_STALLED},
{   0   }
};
...
#else
void    usbx_ux_host_class_hid_mouse_activate_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR *                          memory_pointer;
    ...
    /* Inform user.  */
    printf("Running ux_host_class_hid_mouse_activate Test....................... ");
...
UINT                                status;
UX_HOST_CLASS_HID                   fake_hid;
UX_HOST_CLASS_HID_CLIENT            fake_hid_client;
...
// UX_MEMORY_BLOCK                    *original_cache_safe_memory_block = _ux_system -> ux_system_cache_safe_memory_pool_start;
    ...
    while (hid_client -> ux_host_class_hid_client_local_instance == UX_NULL)
        tx_thread_sleep(10);
#if 0 /* Tested in basic memory tests. */
    /**************************************************/
    ...
    status = _ux_host_class_hid_mouse_activate(&client_command);
    ...
    /** Why direct: _ux_host_class_hid_mouse_activate() is the first one to call _ux_host_class_hid_report_id_get(), **/
	/** and it only fails normally after being called more than once (by trying to get a report when you pass **/
	/** it the last one in the list). **/
    /**************************************************/

    /* Set up command. */
    fake_hid.ux_host_class_hid_client = &fake_hid_client;
    client_command.ux_host_class_hid_client_command_instance = &fake_hid;

    /* Make sure _ux_host_stack_class_instance_verify() in _ux_host_class_hid_report_id_get() doesn't find any classes. */
    ...
    /** Why direct: _ux_host_class_hid_report_callback_register() loops through each input report until it finds the one passed to it;
    /**       the problem is that the one passed to it is always the first input report, so it always finds it. **/
    /**************************************************/
    ...
    /**************************************************/
    /** Test case: _ux_host_class_hid_idle_set() stalled. **/
    /** activate continues, and there may be periodic report start error. **/
    /**************************************************/

    ux_test_hcd_sim_host_set_actions(stall_on_transfer_0);
    status = _ux_host_class_hid_mouse_activate(&client_command);
    if (status != UX_HOST_CLASS_HID_PERIODIC_REPORT_ERROR && status != UX_SUCCESS)
    {
    ...
    /**************************************************/
    /** Test case: _ux_host_class_hid_idle_set() failed. **/
    /**************************************************/
