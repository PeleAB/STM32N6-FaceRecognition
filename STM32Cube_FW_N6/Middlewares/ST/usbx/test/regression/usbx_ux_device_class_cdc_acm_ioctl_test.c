...
#else
void usbx_ux_device_class_cdc_acm_ioctl_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR *                  memory_pointer;
    ...
    printf("Running ux_device_class_cdc_acm_ioctl Test.......................... ");
...
UX_SLAVE_CLASS_CDC_ACM                              *slaves[2];
UINT                                                i;
    ...
    stepinfo(">>>>>>>>>>>>>>>> Test UX_SLAVE_CLASS_CDC_ACM_IOCTL_SET_LINE_STATE\n");
    ...
    new_line_state.ux_slave_class_cdc_acm_parameter_dtr = !line_state.ux_slave_class_cdc_acm_parameter_dtr;
    new_line_state.ux_slave_class_cdc_acm_parameter_rts = !line_state.ux_slave_class_cdc_acm_parameter_rts;

    status = ux_device_class_cdc_acm_ioctl(cdc_acm_slave, UX_SLAVE_CLASS_CDC_ACM_IOCTL_SET_LINE_STATE, &new_line_state);
    if (status != UX_SUCCESS)
    {
        printf("ERROR #%d: ioctl_set_line_state error 0x%x\n", __LINE__, status);
        error_counter ++;
    }
    ...
    if (line_state.ux_slave_class_cdc_acm_parameter_dtr != new_line_state.ux_slave_class_cdc_acm_parameter_dtr)
    {
        printf("ERROR #%d: dtr error\n", __LINE__);
        error_counter ++;
    }
    if (line_state.ux_slave_class_cdc_acm_parameter_rts != new_line_state.ux_slave_class_cdc_acm_parameter_rts)
    {
        printf("ERROR #%d: rts error\n", __LINE__);
        error_counter ++;
    }

    slaves[0] = cdc_acm_slave;
    slaves[1] = cdc_acm_slave1;

    stepinfo(">>>>>>>>>>>>>>>> Test UX_SLAVE_CLASS_CDC_ACM_IOCTL_ABORT_PIPE no pending transfer\n");
    for (i = 0; i < 2; i ++)
    {
        status  = ux_device_class_cdc_acm_ioctl(slaves[i], UX_SLAVE_CLASS_CDC_ACM_IOCTL_ABORT_PIPE, (VOID*)UX_SLAVE_CLASS_CDC_ACM_ENDPOINT_XMIT);
        status |= ux_device_class_cdc_acm_ioctl(slaves[i], UX_SLAVE_CLASS_CDC_ACM_IOCTL_ABORT_PIPE, (VOID*)UX_SLAVE_CLASS_CDC_ACM_ENDPOINT_RCV);
        if (status != UX_SUCCESS)
        {
            printf("ERROR #%d: abort fail 0x%x\n", __LINE__, status);
            error_counter ++;
        }

        status  = ux_device_class_cdc_acm_ioctl(slaves[i], UX_SLAVE_CLASS_CDC_ACM_IOCTL_ABORT_PIPE, (VOID*)0xFF);
        if (status != UX_ENDPOINT_HANDLE_UNKNOWN)
        {
            printf("ERROR #%d: abort code not expected 0x%x\n", __LINE__, status);
            error_counter ++;
        }
    }

    stepinfo(">>>>>>>>>>>>>>>> Test UX_SLAVE_CLASS_CDC_ACM_IOCTL_ABORT_PIPE pending transfer\n");

    /* Start read on another thread.  */
    test_slave_code = 1;
