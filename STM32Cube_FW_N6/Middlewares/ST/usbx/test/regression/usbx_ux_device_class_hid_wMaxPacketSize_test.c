...
#define DEVICE_FRAMEWORK_OFFSET_FS_EP_MPS (DEVICE_FRAMEWORK_LENGTH_FULL_SPEED-HID_TEST_IFC_DESC_ALL_LEN-7-7+4)
...
#define DEVICE_FRAMEWORK_OFFSET_HS_EP_MPS (DEVICE_FRAMEWORK_LENGTH_HIGH_SPEED-HID_TEST_IFC_DESC_ALL_LEN-7+4)
...
#else
void    usbx_ux_device_class_hid_wMaxPacketSize_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR *                          memory_pointer;
    ...
    /* Inform user.  */
    printf("Running ux_device_class_hid_ wMaxPacketSize tests .................. ");
    stepinfo("\n");
...
struct _test {
    ...
    {UX_FULL_SPEED_DEVICE, 0},
    {UX_FULL_SPEED_DEVICE, 128},
    {UX_FULL_SPEED_DEVICE, 8+(2<<11)},
    {UX_FULL_SPEED_DEVICE, 0xFFFF},
    {UX_FULL_SPEED_DEVICE, 16},
    {UX_HIGH_SPEED_DEVICE, 16},
    {UX_HIGH_SPEED_DEVICE, 16+(1<<11)},
    {UX_HIGH_SPEED_DEVICE, 0xFFFF},
    {UX_HIGH_SPEED_DEVICE, 32},
};
#define N_TESTS (sizeof(tests)/sizeof(struct _test))
    ...
    /* No exit on assert hit.  */
    ux_test_assert_hit_exit(0);
    ...
    for (i = 0; i < N_TESTS; i ++)
    {
        ...
        if (tests[i].speed == UX_HIGH_SPEED_DEVICE)
            _ux_utility_short_put(device_framework_high_speed+DEVICE_FRAMEWORK_OFFSET_HS_EP_MPS, tests[i].wMaxPacketSize);
        else
            _ux_utility_short_put(device_framework_full_speed+DEVICE_FRAMEWORK_OFFSET_FS_EP_MPS, tests[i].wMaxPacketSize);
        ...
        /* Set configuration.  */
        _ux_host_stack_configuration_set(device -> ux_device_first_configuration);
    }
