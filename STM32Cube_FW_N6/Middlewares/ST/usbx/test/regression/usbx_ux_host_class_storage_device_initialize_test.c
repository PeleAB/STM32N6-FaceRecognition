...
#else
void    usbx_ux_host_class_storage_device_initialize_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR *                          memory_pointer;
    ...
    /* Inform user.  */
    printf("Running ux_host_class_storage_device_initialize Test................ ");
    stepinfo("\n");
...
ULONG                                       temp;
...
#if 0 // [CQ: moved to _activate 202308]
    stepinfo(">>>>>>>>>>>>>>> ux_host_class_storage_device_initialize - support_check - Protocol FAIL\n");

    temp = storage -> ux_host_class_storage_interface -> ux_interface_descriptor.bInterfaceProtocol;
    storage -> ux_host_class_storage_interface -> ux_interface_descriptor.bInterfaceProtocol = 0xAA;

    status = _ux_host_class_storage_device_initialize(storage);
    if (status != UX_HOST_CLASS_PROTOCOL_ERROR)
    {
        printf("ERROR #%d, code 0x%x\n", __LINE__, status);
        test_control_return(1);
    }

    storage -> ux_host_class_storage_interface -> ux_interface_descriptor.bInterfaceProtocol = temp;
#endif
    ...
    stepinfo(">>>>>>>>>>>>>>> ux_host_class_storage_device_initialize - media_type CDROM SUCCESS/NOT SUPPORTED\n");

    temp = slave_storage -> ux_slave_class_storage_lun[0].ux_slave_class_storage_media_type;
    slave_storage -> ux_slave_class_storage_lun[0].ux_slave_class_storage_media_type = UX_SLAVE_CLASS_STORAGE_MEDIA_CDROM;
    slave_storage -> ux_slave_class_storage_lun[1].ux_slave_class_storage_media_type = 0xF3;
    ...
    stepinfo(">>>>>>>>>>>>>>> ux_host_class_storage_device_initialize - media_type Unsupported\n");
    ...
    slave_storage -> ux_slave_class_storage_lun[0].ux_slave_class_storage_media_type = temp;
