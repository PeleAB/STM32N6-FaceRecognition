...
static UCHAR device_framework_full_speed[] = {
    ...
    /* Configuration descriptor */
        0x09, 0x02, 0x32, 0x00, 0x02, 0x01, 0x00, 0xc0,
        0x32,
    ...
    /* Interface descriptor */
        0x09, 0x04, 0x01, 0x00, 0x00, 0xFF, 0xFF, 0xFF,
        0x00,

    /* Interface descriptor */
        0x09, 0x04, 0x01, 0x01, 0x00, 0xFF, 0xFF, 0xFF,
        0x00
    };


#define DEVICE_FRAMEWORK_LENGTH_HIGH_SPEED 78
...
{
    if (error_code != UX_INTERFACE_HANDLE_UNKNOWN)
    {
...
#else
void    usbx_ux_host_stack_configuration_interface_get_test_application_define(void *first_unused_memory)
#endif
{
...
UX_SLAVE_CLASS_DPUMP_PARAMETER  parameter;
    ...
    /* Inform user.  */
    printf("Running ux_host_stack_configuration_interface_get Test.............. ");
...
UX_INTERFACE    *test_interface;
    ...
    /* At this point, the data pump class has been found.  Now get the interface
       using invalid interface index.  */
    status = _ux_host_stack_configuration_interface_get(dpump -> ux_host_class_dpump_device -> ux_device_first_configuration, 3, 0, &test_interface);
    ...
    /* Now get the interface using invalid alternate setting index.  */
    status = _ux_host_stack_configuration_interface_get(dpump -> ux_host_class_dpump_device -> ux_device_first_configuration, 0, 1, &test_interface);
    if (status != UX_INTERFACE_HANDLE_UNKNOWN)
    {
    ...
    /* Now get the interface using invalid alternate setting index.  */
    status = _ux_host_stack_configuration_interface_get(dpump -> ux_host_class_dpump_device -> ux_device_first_configuration, 1, 2, &test_interface);
    if (status != UX_INTERFACE_HANDLE_UNKNOWN)
    {
