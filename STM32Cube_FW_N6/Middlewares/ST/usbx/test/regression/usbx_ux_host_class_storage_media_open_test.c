...
#else
void    usbx_ux_host_class_storage_media_open_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR *                          memory_pointer;
    ...
    /* Inform user.  */
    printf("Running ux_host_class_storage_media_open Test....................... ");
    stepinfo("\n");
...
FX_MEDIA                                    *media;
...
#if !!defined(UX_HOST_CLASS_STORAGE_NO_FILEX)
    /* In this case, _media_open and _partition_read are called for coverage.  */
    /* They are actually not supported.  */
    _ux_host_class_storage_media_open(storage, 0);
    _ux_host_class_storage_partition_read(storage, 0, 0);
#else
    stepinfo(">>>>>>>>>>>>>>> ux_host_class_storage_media_open - UX_HOST_CLASS_MEMORY_ERROR\n");
    ...
    status = _ux_host_class_storage_media_open(storage, 0);
    if (status != UX_HOST_CLASS_MEMORY_ERROR)
    {
        printf("ERROR #%d, code 0x%x\n", __LINE__, status);
        test_control_return(1);
    }
    ...
    stepinfo(">>>>>>>>>>>>>>> ux_host_class_storage_media_open - UX_MEMORY_INSUFFICIENT\n");

    /* Close a media for test.  */
