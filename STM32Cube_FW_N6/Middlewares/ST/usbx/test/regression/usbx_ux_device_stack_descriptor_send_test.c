...
static UCHAR buffer[UX_SLAVE_REQUEST_CONTROL_MAX_LENGTH * 2];
...
static VOID set_configure_descriptor_length(ULONG length)
{
    device_framework_full_speed[0x12        + 2] = LSB(length);
    device_framework_full_speed[0x12        + 3] = MSB(length);
    device_framework_high_speed[0x12 + 0x0a + 2] = LSB(length);
    device_framework_high_speed[0x12 + 0x0a + 3] = LSB(length);
}
...
#else
void    usbx_ux_device_stack_descriptor_send_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR *                          memory_pointer;
    ...
    /* Inform user.  */
    printf("Running ux_device_stack_descriptor_send test........................ ");
...
UCHAR                          *hid_class_descriptor = device_framework_high_speed + 0x2E;
    ...
    /**************************************************
     * Test case: Get string descriptor with zero language ID. Only tested if UX_DEVICE_ENABLE_GET_STRING_WITH_ZERO_LANGUAGE_ID is defined.
     **************************************************/
    string_framework[3] = 12;
    /* Create a transfer request for getting the string descriptor with zero .  */
    ...
    transfer_request -> ux_transfer_request_value =             UX_STRING_DESCRIPTOR_ITEM << 8 | 3;
    ...
    string_framework[3] = 0xff;
#endif

    /* Modify configuration descriptor total length to generate error.  */
    set_configure_descriptor_length(UX_SLAVE_REQUEST_CONTROL_MAX_LENGTH+1);

    /**************************************************
     * Test case: 'case: UX_OTHER_SPEED_DESCRIPTOR_ITEM'
    ...
    /**************************************************
     * Test case: 'case: UX_CONFIGURATION_DESCRIPTOR_ITEM'
     *     'if (length > UX_SLAVE_REQUEST_CONTROL_MAX_LENGTH)'
     **************************************************/
    ...
    /**************************************************
     * Test case: language ID framework is too large for device to send.
     **************************************************/

    /* Make the language ID framework too large.  */
    _ux_system_slave->ux_system_slave_language_id_framework_length = UX_SLAVE_REQUEST_CONTROL_MAX_LENGTH + 1;

    /* Create a transfer request for the HID class descriptor.  */
    transfer_request -> ux_transfer_request_data_pointer =      buffer;
    transfer_request -> ux_transfer_request_requested_length =  0xffff;
    transfer_request -> ux_transfer_request_function =          UX_GET_DESCRIPTOR;
    transfer_request -> ux_transfer_request_type =              UX_REQUEST_IN | UX_REQUEST_TYPE_STANDARD | UX_REQUEST_TARGET_DEVICE;
    transfer_request -> ux_transfer_request_value =             UX_STRING_DESCRIPTOR_ITEM << 8 | 0;
    transfer_request -> ux_transfer_request_index =             0;
