...
static UX_TEST_SIM_ENTRY_ACTION dcd_endpoint_status_fail[] = {
/* function, request to match,
   port action, port status,
   request action, request EP, request data, request actual length, request status,
   status, additional callback,
   no_return */
{   UX_DCD_ENDPOINT_STATUS, NULL,
        UX_FALSE, 0,
        0         , 0, UX_NULL, 0, 0,
        UX_ERROR , UX_NULL},
{   0   }
...
#else
void    usbx_ux_device_stack_get_status_test_application_define(void *first_unused_memory)
#endif
{
...
UX_SLAVE_CLASS_HID_PARAMETER    hid_parameter;
    ...
    printf("Running ux_device_stack_get_status Test............................. ");
...
UX_TRANSFER             *transfer_request;
    ...
    /* Get device, control endpoint and transfer request. */
    ...
    if (buffer[0] != 0)
    {
        ...
        printf("ERROR #%d: GetDeviceStatus %x but expect 0\n", __LINE__, buffer[0]);
        error_counter ++;
    }

    expect_errors = UX_TRUE;

    /* Issue GetEndpointStatus.  */
    ux_test_dcd_sim_slave_set_actions(dcd_endpoint_status_fail);
    ...
    if (status != UX_TRANSFER_STALLED)
    {
        ...
        printf("ERROR #%d: GetEndpointStatus(0x81) expect UX_TRANSFER_STALLED but got 0x%x\n", __LINE__, status);
        error_counter ++;
    }
