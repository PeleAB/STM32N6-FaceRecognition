...
#else
void    usbx_msrc_72525_pima_obj_handles_get_test_application_define(void *first_unused_memory)
#endif
{
...
ULONG                   test_n;
    ...
    /* Inform user.  */
    printf("Running MSRC 72525 PIMA obj handles get Test........................ ");
...
VOID test_pima_object_handles_send(UX_SLAVE_CLASS_PIMA *pima, ULONG array_length, ULONG container_length)
{
...
static ULONG            replace_pima_obj_n = 0;
...
void  replace_pima_cmd_thread_entry(ULONG arg)
{
    while(1)
    {
        ...
        test_pima_object_handles_send(pima_device, replace_pima_obj_n, replace_pima_container_l);
    }
}
...
ULONG actual_length;
    ...
    /* nb_objects check overflow!  */
    pima_host_session.ux_host_class_pima_session_nb_objects = 0x40000001; /* Overflow if * 4 (=4).  */
...
/*
The _ux_host_class_pima_object_handles_get function does not process the
response retrieved from a pima device in a secure manner. The number of returned
object handles may be maliciously crafted to bypass validation against available
buffer size by triggering an integer overflow in multiplication of number of
object handles and size of ULONG type.

A large number of object handles, stored in the nb_object_handles variable, will
result in a buffer overflow in the for loop responsible for unpacking of object
handles.
...
if (status == UX_SUCCESS)
{
    ...
    if ((nb_object_handles * sizeof(ULONG)) > object_handles_length)
...
For example if the application calls _ux_host_class_pima_object_handles_get to
retrieve handles for two objects to the object_handles_array array with
object_handles_length set to 8 bytes, setting nb_object_handles to 1073741825
will bypass the memory check and overflow the target array.

*Impact*

Processing of a maliciously crafted UX_HOST_CLASS_PIMA_OC_GET_OBJECT_HANDLES
request response will result in integer and buffer overflows. The attacker may
exploit this issue to cause denial of service by crashing the pima host.
Depending on end application it may also be possible to achieve execution of
arbitrary code i.e., by providing the payload in a larger usb transfer prior to
retrieval of PIMA object handles reusing the memory region for buffer allocation.

*Reproduction steps*

- Connect a malicious pima device to the host
- Provide a crafted response to the UX_HOST_CLASS_PIMA_OC_GET_OBJECT_HANDLES request triggering an integer overflow during multiplication nb_object_handles and sizeof(ULONG)
- Observe that (nb_object_handles * sizeof(ULONG)) > object_handles_length) condition may be bypassed by exploiting the integer overflow
- Observe further overflow of the object_handles_array array in the for loop responsible for unpacking of object handles
...
*/
    ...
    /* Device reported array size (nb_object_handles * sizeof(ULONG)) overflow!  */
    replace_pima_obj_n = 0x40000001; /* Overflow if * 4 (=4).  */
