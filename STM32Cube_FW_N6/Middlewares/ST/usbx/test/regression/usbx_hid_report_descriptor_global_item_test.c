/* This test ensures that certain data-descriptive global items are correct within reports. This includes:
    ...
    -UNIT
    -UNIT_EXPONENT
    -INPUT/OUTPUT/FEATURE bits
    -We do not check usages here because they are checked in another test (via decompress).

   Reasoning: The user needs to know these in order to interpret the data in the report.
 */
...
static UCHAR hid_report_descriptor[] = {
    ...
    0x55, 0x67,                    //   UNIT_EXPONENT (0)
    0x75, 0x01,                    //   REPORT_SIZE (1)
    0x95, 0x08,                    //   REPORT_COUNT (8)
    0x82, 0xab, 0x01,              //   INPUT (chosen specifically for testing)
...
#else
void    usbx_hid_report_descriptor_global_item_test_application_define(void *first_unused_memory)
#endif
{
...
UINT                            descriptor_size = HID_REPORT_LENGTH;
    ...
    /* Inform user.  */
    printf("Running HID Report Descriptor Global Item Test...................... ");
...
UX_HOST_CLASS_HID_FIELD             *report_field;
    ...
    if(
       report_field -> ux_host_class_hid_field_logical_min != 0x7f ||
       report_field -> ux_host_class_hid_field_logical_max != 0xff ||
       report_field -> ux_host_class_hid_field_physical_min != 0x00 ||
       report_field -> ux_host_class_hid_field_physical_max != 0xff ||
       report_field -> ux_host_class_hid_field_unit != 0x14 ||
       report_field -> ux_host_class_hid_field_unit_expo != 0x67 ||
       report_field -> ux_host_class_hid_field_report_id != 0x33 ||

       (report_field -> ux_host_class_hid_field_value & UX_HOST_CLASS_HID_ITEM_CONSTANT) == 0 ||
       (report_field -> ux_host_class_hid_field_value & UX_HOST_CLASS_HID_ITEM_VARIABLE) == 0 ||
       (report_field -> ux_host_class_hid_field_value & UX_HOST_CLASS_HID_ITEM_RELATIVE) != 0 ||
       (report_field -> ux_host_class_hid_field_value & UX_HOST_CLASS_HID_ITEM_WRAP) == 0 ||
       (report_field -> ux_host_class_hid_field_value & UX_HOST_CLASS_HID_ITEM_NON_LINEAR) != 0 ||
       (report_field -> ux_host_class_hid_field_value & UX_HOST_CLASS_HID_ITEM_NO_PREFERRED_STATE) == 0 ||
       (report_field -> ux_host_class_hid_field_value & UX_HOST_CLASS_HID_ITEM_NULL_STATE) != 0 ||
       (report_field -> ux_host_class_hid_field_value & UX_HOST_CLASS_HID_ITEM_VOLATILE) == 0 ||
       (report_field -> ux_host_class_hid_field_value & UX_HOST_CLASS_HID_ITEM_BUFFERED_BYTES) == 0
       )
    {
        printf("Error on line %d\n", __LINE__);
        test_control_return(1);
