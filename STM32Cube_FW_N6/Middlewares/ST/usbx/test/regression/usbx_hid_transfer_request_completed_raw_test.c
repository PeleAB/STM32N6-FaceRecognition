...
static VOID error_callback(UINT system_level, UINT system_context, UINT error_code)
{
    ...
    if (error_code != UX_BUFFER_OVERFLOW)
    {
...
#else
void    usbx_hid_transfer_request_completed_raw_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR *                          memory_pointer;
    ...
    /* Inform user.  */
    printf("Running HID Transfer Request Completed Raw Test..................... ");
...
#define MAX_NUM_CALLBACKS 16

VOID  dummy_transfer_request_callback_successful(UX_HOST_CLASS_HID_REPORT_CALLBACK *callback)
{

    static INT  expected_value = -1;
    ULONG       actual_value;

    actual_value = *((ULONG *)callback->ux_host_class_hid_report_callback_buffer);
...
VOID  dummy_transfer_request_callback_failure(UX_HOST_CLASS_HID_REPORT_CALLBACK *callback)
{
...
UCHAR                               buffer[1024];
    ...
    /**************************************************/
    /** Test case: successful raw report callback. **/
    /**************************************************/
    ...
    callback.ux_host_class_hid_report_callback_function =   dummy_transfer_request_callback_successful;
    callback.ux_host_class_hid_report_callback_buffer =     buffer;
    callback.ux_host_class_hid_report_callback_flags =      UX_HOST_CLASS_HID_REPORT_RAW;
    callback.ux_host_class_hid_report_callback_length =     sizeof(buffer);

    ux_host_class_hid_report_callback_register(hid, &callback);

    /* Wait until callback has been invoked at least MAX_NUM_CALLBACKS times. */
    while(num_callbacks < MAX_NUM_CALLBACKS)
        tx_thread_sleep(10);
#if 0 /* User buffer is not assigned on initialize, see USBX-283 */
    /**************************************************/
    /** Test case: callback buffer too small. **/
    /**************************************************/

    /* Change the length to a small value.  */
    callback.ux_host_class_hid_report_callback_function =   dummy_transfer_request_callback_failure;
    callback.ux_host_class_hid_report_callback_length = 1;

    ux_host_class_hid_report_callback_register(hid, &callback);

    /* Wait until we receive the expected error. */
    while(callback_error_code != UX_BUFFER_OVERFLOW)
        tx_thread_sleep(10);
#endif
...
UINT                            i;
    ...
    *((ULONG *)hid_event.ux_device_class_hid_event_buffer) = 0;
    ...
    while (1)
    {
        ...
        for (i = 0; i < MAX_NUM_CALLBACKS; i++)
        {
            ...
            /* Increment.  */
            *((ULONG *)hid_event.ux_device_class_hid_event_buffer) += 1;
        }
    }
}
