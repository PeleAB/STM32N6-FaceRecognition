...
#else
void    usbx_ux_device_class_storage_synchronize_cache_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR *                          memory_pointer;
    ...
    /* Inform user.  */
    printf("Running ux_device_class_storage_synchronize_cache Test.............. ");
    stepinfo("\n");
...
UX_DEVICE                                   *device;
    ...
    _test_init_cbw_SYNCHRONIZE_CACHE(UX_FALSE, UX_FALSE, 0, 1);
    ram_disk_flush_status = UX_ERROR;
    status = _test_send_cbw();
    if (status != UX_SUCCESS)
    {
        printf("ERROR #%d: code 0x%x\n", __LINE__, status);
    ...
    ram_disk_flush_status = UX_SUCCESS;
