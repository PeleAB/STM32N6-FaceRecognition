...
#else
void usbx_ux_host_stack_class_instance_verify_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR *                  memory_pointer;
    ...
    printf("Running ux_host_stack_class_instance_verify Test.................... ");
#else
    printf("Running ux_host_stack_class_instance_verify Test................SKIP SUCCESS!\n");
...
UINT                                                status;
UCHAR                                               invalid_name[UX_MAX_CLASS_NAME_LENGTH + 2];

#if !defined(UX_NAME_REFERENCED_BY_POINTER)
    stepinfo(">>>>>>>>>>>>>>>> Test _ux_host_stack_class_instance_verify - class name C string errort\n");
    error_callback_ignore = UX_TRUE;
    _ux_utility_memory_set(invalid_name, ' ', sizeof(invalid_name));
    _ux_utility_memory_copy(invalid_name, _ux_system_host_class_cdc_acm_name, 21);
    status = _ux_host_stack_class_instance_verify(invalid_name, cdc_acm_host_control);
    if (status != UX_ERROR)
    {
        printf("ERROR #%d: expect UX_ERROR but got 0x%x\n", __LINE__, status);
        error_counter ++;
    }
    ...
    stepinfo(">>>>>>>>>>>>>>>> Test _ux_host_stack_class_instance_verify - no class driver\n");

    status = _ux_host_stack_class_instance_verify(_ux_system_host_class_cdc_acm_name, cdc_acm_host_control);
    if (status != UX_HOST_CLASS_INSTANCE_UNKNOWN)
    {
        printf("ERROR #%d: expect UX_HOST_CLASS_INSTANCE_UNKNOWN but got 0x%x\n", __LINE__, status);
        error_counter ++;
    }
    ...
    stepinfo(">>>>>>>>>>>>>>>> Test _ux_host_stack_class_instance_verify - no class driver active\n");
    ...
    stepinfo(">>>>>>>>>>>>>>>> Test _ux_host_stack_class_instance_verify - instance no match\n");

    status = _ux_host_stack_class_instance_verify(_ux_system_host_class_cdc_acm_name, (VOID *)4);
    if (status != UX_HOST_CLASS_INSTANCE_UNKNOWN)
    {
        printf("ERROR #%d: expect UX_HOST_CLASS_INSTANCE_UNKNOWN but got 0x%x\n", __LINE__, status);
        error_counter ++;
    ...
    stepinfo(">>>>>>>>>>>>>>>> Test _ux_host_stack_class_instance_verify - class name not match\n");

    status = _ux_host_stack_class_instance_verify(_ux_system_host_class_hid_name, cdc_acm_host_control);
    if (status != UX_HOST_CLASS_INSTANCE_UNKNOWN)
    {
        printf("ERROR #%d: expect UX_HOST_CLASS_INSTANCE_UNKNOWN but got 0x%x\n", __LINE__, status);
        error_counter ++;
