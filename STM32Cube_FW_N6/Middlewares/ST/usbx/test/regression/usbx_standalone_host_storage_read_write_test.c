...
static UCHAR                            ram_disk_rw_disconnect = 0;/* device disconnect while read/write in progress */
static ULONG                            ram_disk_rw_disconnect_count = 0;
...
#else
void    usbx_standalone_host_storage_read_write_test_application_define(void *first_unused_memory)
#endif
{
...
ULONG                           test_n;
    ...
    /* Inform user.  */
    printf("Running STANDALONE Host Storage Read Write Test..................... ");
...
static void _msc_media_write_read_disconnect_test(const char* __file__, int __line__)
{
UINT        status;
ULONG       test_size = 1;
UINT        i;
    ...
    stepinfo(">>>>>>>>>>>> Device disconnect on writing test\n");
    ram_disk_rw_disconnect = 1;
    ram_disk_rw_disconnect_count = 0;
    for(i = 0; i < test_size * 512; i ++)
        buffer[i] = (UCHAR)(i + (i >> 8));
    status = _media_driver_write(media, _ux_host_class_storage_driver_entry,
            buffer, 48, test_size);
    ...
    if (status == FX_SUCCESS)
    {
        printf("%s:%d:%d expect write fail\n", __file__, __line__, __LINE__);
        test_control_return(1);
    }
    if (ram_disk_rw_disconnect_count != 1)
    {
        printf("%s:%d:%d expect write once\n", __file__, __line__, __LINE__);
        test_control_return(1);
    }
    if (host_storage_instance_get(0) == UX_SUCCESS)
    {
        printf("%s:%d:%d expect device disconnect\n", __file__, __line__, __LINE__);
        test_control_return(1);
    }

    stepinfo(">>>>>>>>>>>> Device connect after disconnect on writing check\n");
    error_callback_counter = 0;
    ram_disk_rw_disconnect = 0;
    ux_test_dcd_sim_slave_connect(UX_FULL_SPEED_DEVICE);
    ux_test_hcd_sim_host_connect(UX_FULL_SPEED_DEVICE);

    /* Wait and break on errors. */
    ux_test_breakable_sleep(1000, sleep_break_on_connect);

    /* Check error */
    if (host_storage_instance_get(0) != UX_SUCCESS)
    {
        printf("%s:%d:%d device re-connect fail\n", __file__, __line__, __LINE__);
        test_control_return(1);
    }

    stepinfo(">>>>>>>>>>>> Disk disconnect on reading test\n");
    ram_disk_rw_disconnect = 1;
    ram_disk_rw_disconnect_count = 0;
    _ux_utility_memory_set(buffer, 0x00, test_size * 512);
    ram_disk_rw_wait_start = tx_time_get();
    ...
    stepinfo(">>>>>>>>>>>> Device connect after disconnect on reading check\n");
    error_callback_counter = 0;
    ram_disk_rw_disconnect = 0;
    ux_test_dcd_sim_slave_connect(UX_FULL_SPEED_DEVICE);
    ux_test_hcd_sim_host_connect(UX_FULL_SPEED_DEVICE);
    ...
    if (host_storage_instance_get(0) != UX_SUCCESS)
    {
        stepinfo("%s:%d:%d device re-connect fail\n", __file__, __line__, __LINE__);
        test_control_return(1);
    }
}
...
UINT                                        status;
    ...
    _msc_media_write_read_disconnect_test(__FILE__, __LINE__);
...
UINT    status =  0;
    ...
    if (ram_disk_rw_disconnect)
    {
        ram_disk_rw_disconnect_count ++;
        ux_test_dcd_sim_slave_disconnect();
        ux_test_hcd_sim_host_disconnect();
        return(UX_ERROR);
    }
