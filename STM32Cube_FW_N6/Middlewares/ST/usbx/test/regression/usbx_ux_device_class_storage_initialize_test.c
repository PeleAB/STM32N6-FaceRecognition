...
#else
void    usbx_ux_device_class_storage_initialize_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR *                          memory_pointer;
    ...
    /* Inform user.  */
    printf("Running ux_device_class_storage_initialize Test..................... ");
    stepinfo("\n");
...
static UINT _test_clear_stall(UCHAR clear_read_stall)
{
...
UX_SLAVE_CLASS                              *slave_class;
    ...
    stepinfo(">>>>>>>>>>>>>>> ux_device_class_storage_initialize - lun exceed UX_MAX_SLAVE_LUN\n");
    global_storage_parameter.ux_slave_class_storage_parameter_number_lun = UX_MAX_SLAVE_LUN + 1;
    status =  ux_device_stack_class_register(_ux_system_slave_class_storage_name, ux_device_class_storage_entry,
                                            1, 1, (VOID *)&global_storage_parameter);
    if (status == UX_SUCCESS)
    {
    ...
    stepinfo(">>>>>>>>>>>>>>> ux_device_class_storage_initialize - storage memory error\n");
    ux_test_utility_sim_mem_allocate_until(0);
    status =  ux_device_stack_class_register(_ux_system_slave_class_storage_name, ux_device_class_storage_entry,
                                            1, 1, (VOID *)&global_storage_parameter);
    ...
    stepinfo(">>>>>>>>>>>>>>> ux_device_class_storage_initialize - thread stack memory error\n");
    ux_test_utility_sim_mem_allocate_until(sizeof(UX_SLAVE_CLASS_STORAGE) + UX_THREAD_STACK_SIZE / 2);
    status =  ux_device_stack_class_register(_ux_system_slave_class_storage_name, ux_device_class_storage_entry,
                                            1, 1, (VOID *)&global_storage_parameter);
    ux_test_utility_sim_mem_free_all();
    if (status == UX_SUCCESS)
    ...
    stepinfo(">>>>>>>>>>>>>>> ux_device_class_storage_initialize - thread create error\n");
    slave_device =  &_ux_system_slave->ux_system_slave_device;
    slave_class = &_ux_system_slave->ux_system_slave_class_array[1];
    status =  _ux_utility_thread_create(&slave_class -> ux_slave_class_thread, "ux_slave_storage_thread",
                                        _ux_device_class_storage_thread,
                                        (ULONG) (ALIGN_TYPE) slave_class, (VOID *) buffer,
    ...
    _ux_utility_thread_delete(&slave_class -> ux_slave_class_thread);

    stepinfo(">>>>>>>>>>>>>>> ux_device_class_storage_initialize - lun block length error\n");
    global_storage_parameter.ux_slave_class_storage_parameter_lun[0].ux_slave_class_storage_media_block_length = UX_SLAVE_CLASS_STORAGE_BUFFER_SIZE * 2;
    status =  ux_device_stack_class_register(_ux_system_slave_class_storage_name, ux_device_class_storage_entry,
                                            1, 1, (VOID *)&global_storage_parameter);
    if (status == UX_SUCCESS)
    {
