...
static UCHAR                        demo_buffer[UX_DEMO_BUFFER_SIZE];
...
#else
void    usbx_ux_device_class_storage_test_ready_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR *                          memory_pointer;
    ...
    /* Inform user.  */
    printf("Running ux_device_class_storage_test_ready Test..................... ");
    stepinfo("\n");
...
UX_HOST_CLASS_STORAGE_MEDIA                 *storage_media;
    ...
    /* Change RAM disk status and wait it sent.  */
    ram_disk_status = UX_ERROR;
    ram_disk_media_status = UX_SLAVE_CLASS_STORAGE_SENSE_KEY_NOT_READY;
    ram_disk_status_sent = UX_FALSE;
    while(!ram_disk_status_sent)
        _ux_utility_delay_ms(100);

    /* Confirm media unmounted on host side.  */
    status = storage_media_status_wait(storage_media, UX_HOST_CLASS_STORAGE_MEDIA_UNMOUNTED, 1000);
    if (status != UX_SUCCESS)
    {
        printf("ERROR #%d\n", __LINE__);
        test_control_return(1);
    ...
    /* Confirm media mounted on host side.  */
    status = storage_media_status_wait(storage_media, UX_HOST_CLASS_STORAGE_MEDIA_MOUNTED, 1000);
    if (status != UX_SUCCESS)
    {
        printf("ERROR #%d\n", __LINE__);
        test_control_return(1);
    ...
    status = _test_transfer_data(demo_buffer, 64, UX_FALSE);
