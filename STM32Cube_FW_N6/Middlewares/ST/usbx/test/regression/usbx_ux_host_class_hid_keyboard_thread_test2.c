...
/**	            _ux_utility_semaphore_get() fail. Either way, something else in USBX is going to break... **/
/** How: we run the call in a separate thread since the ux_host_class_hid_keyboard_thread never returns. **/
/**************************************************/
...
static VOID error_callback(UINT system_level, UINT system_context, UINT error_code)
{
    ...
    if (error_code != UX_HOST_CLASS_INSTANCE_UNKNOWN)
    {
...
#else
void    usbx_ux_host_class_hid_keyboard_thread_test2_application_define(void *first_unused_memory)
#endif
{
...
UINT                            descriptor_size = HID_REPORT_LENGTH;
    ...
    /* Inform user.  */
    printf("Running ux_host_class_hid_keyboard_thread Test 2.................... ");
...
UINT                                status;
UX_HOST_CLASS_HID                   keyboard_hid;


    /* Since keyboard_hid isn't located in a USBX class container, instance_veryify() will fail. */
	keyboard.ux_host_class_hid_keyboard_hid = &keyboard_hid;

    /* Create the semaphore used in _ux_host_class_hid_keyboard_thread(). */
	status = ux_utility_semaphore_create(&keyboard.ux_host_class_hid_keyboard_semaphore, "dummy", 1);
	if (status != UX_SUCCESS)
	{
    ...
    UX_THREAD_EXTENSION_PTR_SET(tx_thread_identify(), &keyboard)
	_ux_host_class_hid_keyboard_thread((ULONG)(ALIGN_TYPE)&keyboard);
}
...
UINT        status;
    ...
    /* Give test thread time to make call. */
	tx_thread_sleep(20);
