...
static UINT error_counts = 0;
static UINT last_error = UX_SUCCESS;
...
#else
void    usbx_ux_host_class_hid_transfer_request_completed_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR *                          memory_pointer;
    ...
    /* Inform user.  */
    printf("Running ux_host_class_hid_transfer_request_completed Test........... ");
...
UX_HOST_CLASS_HID_KEYBOARD     *keyboard;
    ...
    transfer_request.ux_transfer_request_class_instance = hid;
    transfer_request.ux_transfer_request_completion_code = UX_SUCCESS;

    ux_test_utility_sim_mem_alloc_fail_all_start();

    _ux_host_class_hid_transfer_request_completed(&transfer_request);

    /* Restore state for next test. */
    ux_test_utility_sim_mem_alloc_fail_all_stop();

    /**************************************************/
    /** Test case: if (hid_report -> ux_host_class_hid_report_callback_function != UX_NULL) fails **/
    /**************************************************/

    tmp = (ALIGN_TYPE)hid -> ux_host_class_hid_parser.ux_host_class_hid_parser_input_report -> ux_host_class_hid_report_callback_function;
    hid -> ux_host_class_hid_parser.ux_host_class_hid_parser_input_report -> ux_host_class_hid_report_callback_function = UX_NULL;

    /* Wait for transfer_completed() to run. */
    tx_thread_sleep(50);

    /* Restore state for next test. */
    hid -> ux_host_class_hid_parser.ux_host_class_hid_parser_input_report -> ux_host_class_hid_report_callback_function = (VOID (*) (struct UX_HOST_CLASS_HID_REPORT_CALLBACK_STRUCT *))tmp;

    /**************************************************/
    /** Test case: (hid_report->ux_host_class_hid_report_number_item * 8) overflow **/
    /**************************************************/

    tmp = hid -> ux_host_class_hid_parser.ux_host_class_hid_parser_input_report -> ux_host_class_hid_report_number_item;

    last_error = UX_SUCCESS;
    hid -> ux_host_class_hid_parser.ux_host_class_hid_parser_input_report -> ux_host_class_hid_report_number_item = 0xFFFFFFFF;

    /* Wait for transfer_completed() to run. */
    tx_thread_sleep(50);

    if (last_error != UX_MEMORY_INSUFFICIENT)
    {
        printf("Error on line %d, expect error code: %d\n", __LINE__, last_error);
        test_control_return(1);
    }

    /* Restore state for next test. */
    hid -> ux_host_class_hid_parser.ux_host_class_hid_parser_input_report -> ux_host_class_hid_report_number_item = (ULONG)tmp;
