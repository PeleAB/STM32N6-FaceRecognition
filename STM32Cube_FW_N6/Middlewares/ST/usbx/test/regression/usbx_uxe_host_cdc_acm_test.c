...
#else
void usbx_uxe_host_cdc_acm_test_application_define(void *first_unused_memory)
#endif
{
...
{
UINT                                            status;
UX_HOST_CLASS_CDC_ACM                           dummy_cdc_acm_inst;
UX_HOST_CLASS_CDC_ACM                           *dummy_cdc_acm = &dummy_cdc_acm_inst;
UX_HOST_CLASS_CDC_ACM_RECEPTION                 dummy_reception;
UCHAR                                           dummy_buffer[64];
ULONG                                           dummy_ul;

    /* ux_host_class_cdc_acm_read()  */
    status = ux_host_class_cdc_acm_read(UX_NULL, dummy_buffer, 8, &dummy_ul);
    UX_TEST_CHECK_CODE(UX_INVALID_PARAMETER, status);
    status = ux_host_class_cdc_acm_read(dummy_cdc_acm, UX_NULL, 8, &dummy_ul);
    UX_TEST_CHECK_CODE(UX_INVALID_PARAMETER, status);
    status = ux_host_class_cdc_acm_read(dummy_cdc_acm, dummy_buffer, 8, UX_NULL);
    UX_TEST_CHECK_CODE(UX_INVALID_PARAMETER, status);

    /* ux_host_class_cdc_acm_write()  */
    status = ux_host_class_cdc_acm_write(UX_NULL, dummy_buffer, 8, &dummy_ul);
    UX_TEST_CHECK_CODE(UX_INVALID_PARAMETER, status);
    status = ux_host_class_cdc_acm_write(dummy_cdc_acm, UX_NULL, 8, &dummy_ul);
    UX_TEST_CHECK_CODE(UX_INVALID_PARAMETER, status);
    status = ux_host_class_cdc_acm_write(dummy_cdc_acm, dummy_buffer, 8, UX_NULL);
    UX_TEST_CHECK_CODE(UX_INVALID_PARAMETER, status);

    /* ux_host_class_cdc_acm_ioctl()  */
    status = ux_host_class_cdc_acm_ioctl(UX_NULL, 0, UX_NULL);
    UX_TEST_CHECK_CODE(UX_INVALID_PARAMETER, status);

    /* ux_host_class_cdc_acm_reception_start()  */
    status = ux_host_class_cdc_acm_reception_start(UX_NULL, &dummy_reception);
    UX_TEST_CHECK_CODE(UX_INVALID_PARAMETER, status);
    status = ux_host_class_cdc_acm_reception_start(dummy_cdc_acm, UX_NULL);
    UX_TEST_CHECK_CODE(UX_INVALID_PARAMETER, status);

    /* ux_host_class_cdc_acm_reception_stop()  */
    status = ux_host_class_cdc_acm_reception_stop(UX_NULL, &dummy_reception);
    UX_TEST_CHECK_CODE(UX_INVALID_PARAMETER, status);
    status = ux_host_class_cdc_acm_reception_stop(dummy_cdc_acm, UX_NULL);
    UX_TEST_CHECK_CODE(UX_INVALID_PARAMETER, status);
