...
static UCHAR                        host_bulk_out_busy[UX_DEMO_MAX_MESSAGE_LENGTH];
...
static struct _SLOT_TESTER {
    TX_SEMAPHORE semaphore;
    UINT         state;
    ULONG        timeout;
    ULONG        flags;
} slot_tester[UX_DEMO_MAX_SLOT_INDEX + 1];
#define SLOT_TESTER_FLAG_ABORT      (1u)
#define SLOT_TESTER_STATE_IDLE      (0u)
#define SLOT_TESTER_STATE_START     (1u)
#define SLOT_TESTER_STATE_WAIT      (2u)
#define SLOT_TESTER_STATE_NEXT      (3u)
...
#else
void    usbx_host_class_ccid_busy_abort_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR *                  memory_pointer;
    ...
    printf("Running CCID Busy and Abort Test.................................... ");
    ...
    for (int i = 0; i <= UX_DEMO_MAX_SLOT_INDEX; i ++)
    {
        slot_tester[i].timeout = TX_WAIT_FOREVER;
        status = tx_semaphore_create(&slot_tester[i].semaphore, "slot_test_semaphore", 0);
...
static UINT _ccid_abort(UCHAR bSlot, UCHAR bSeq)
{
...
UCHAR           cmd[10], rsp[10];
    /* Issue Abort request.  */
    transfer->ux_transfer_request_type              = 0x21;
    transfer->ux_transfer_request_function          = 0x01;
    transfer->ux_transfer_request_index             = interface->ux_interface_descriptor.bInterfaceNumber;
    transfer->ux_transfer_request_value             = bSlot | (bSeq << 8);
    transfer->ux_transfer_request_requested_length  = 0;
    ...
    /* Issue Abort Message.  */
    _ux_utility_memory_set(cmd, 0, 10);
    cmd[0] = 0x72;
    cmd[5] = bSlot;
    cmd[6] = bSeq;
    status = _ccid_message_bulk_out_in(cmd, 10, rsp, &length);
    return(status);
}

#define _TEST_CCID_BULK_OUT_CHECK(h, l, b) \
    ...
    if ((h) != UX_NULL) { \
        ...
        if (b) { \
            UX_TEST_ASSERT(_ux_utility_memory_compare(device_ccid_callback_log.buf, (b), (l)) == UX_SUCCESS); \
        } \
    }
...
UINT        status;
    ...
    _TEST_CCID_BULK_OUT_CHECK(ux_test_ccid_icc_power_on, 10, host_bulk_out)
    ...
    device_ccid_callback_log.handle = UX_NULL;
    status = _ccid_message_bulk_out_in(host_bulk_out, 10, host_bulk_in, &host_bulk_in_length);
    _TEST_CCID_BULK_OUT_CHECK(ux_test_ccid_icc_power_off, 10, host_bulk_out)
    ...
    device_ccid_callback_log.handle = UX_NULL;
    status = _ccid_message_bulk_out_in(host_bulk_out, 10, host_bulk_in, &host_bulk_in_length);
    _TEST_CCID_BULK_OUT_CHECK(UX_NULL, 10, host_bulk_out)
...
static VOID _ccid_invalid_slot_test(VOID)
{
UINT        status;
    /* PC_to_RDR_IccPowerOn, RDR_to_PC_DataBlock   */
    ...
    status = _ccid_message_bulk_out_in(host_bulk_out, 10, host_bulk_in, &host_bulk_in_length);
    _TEST_CCID_BULK_OUT_CHECK(ux_test_ccid_icc_power_on, 10, host_bulk_out)
    _TEST_CCID_BULK_IN_HEADER_CHECK(0x80, 0, 1, 0, 0, 0)
#endif

#if UX_DEVICE_CLASS_CCID_MAX_N_SLOTS > 2

    /* Good slot.  */
    host_bulk_out[5] = 2; /* bSlot.  */
    device_ccid_callback_log.handle = UX_NULL;
    status = _ccid_message_bulk_out_in(host_bulk_out, 10, host_bulk_in, &host_bulk_in_length);
    _TEST_CCID_BULK_OUT_CHECK(ux_test_ccid_icc_power_on, 10, host_bulk_out)
    _TEST_CCID_BULK_IN_HEADER_CHECK(0x80, 0, 2, 0, 0, 0)
#endif

#if UX_DEVICE_CLASS_CCID_MAX_N_SLOTS > 3

    /* Invalid slot.  */
    host_bulk_out[5] = 3; /* bSlot.  */
    device_ccid_callback_log.handle = UX_NULL;
    status = _ccid_message_bulk_out_in(host_bulk_out, 10, host_bulk_in, &host_bulk_in_length);
    _TEST_CCID_BULK_OUT_CHECK(UX_NULL, 10, host_bulk_out)
    _TEST_CCID_BULK_IN_HEADER_CHECK(0x80, 0, 3, 0, 0x42, 5)
#endif
}

static VOID _ccid_busy_slot_test(VOID)
{
UINT        status;
    stepinfo(">>>>>>>>>> Test CCID Busy ...\n");

    /* PC_to_RDR_XfrBlock, RDR_to_PC_DataBlock.  */
    _ux_utility_memory_set(host_bulk_out, 0, 10);
    _ux_utility_memory_set(host_bulk_out_busy, 0, 10);
    /* Same slot response busy.  */
    {
        stepinfo(">>>>>>>>>> Test CCID Busy - Xfer blocked\n");
        /* _XfrBlock ... blocked.  */
        ...
        _ux_utility_long_put(host_bulk_out+1, 0);
        device_ccid_callback_log.handle = UX_NULL;
        status = _ccid_message_bulk_out_in(host_bulk_out, 10, UX_NULL, &host_bulk_in_length);
        _TEST_CCID_BULK_OUT_CHECK(ux_test_ccid_xfr_block, 10, host_bulk_out)

        stepinfo(">>>>>>>>>> Test CCID Busy - Xfer response busy\n");
        /* _XfrBlock ... response busy.  */
        host_bulk_out_busy[0] = 0x6F;
        host_bulk_out_busy[5] = 0; /* bSlot.  */
        host_bulk_out_busy[6] = 8; /* bSeq.   */
        _ux_utility_long_put(host_bulk_out_busy+1, 0);
        host_bulk_out_busy[6] = 9; /* bSeq.   */
        device_ccid_callback_log.handle = UX_NULL;
        status = _ccid_message_bulk_out_in(host_bulk_out_busy, 10, host_bulk_in, &host_bulk_in_length);
#if defined(UX_DEVICE_STANDALONE)
        _TEST_CCID_BULK_OUT_CHECK(ux_test_ccid_xfr_block, 10, host_bulk_out) /* Task is polled */
#else
        _TEST_CCID_BULK_OUT_CHECK(UX_NULL, 10, host_bulk_out)
#endif
        _TEST_CCID_BULK_IN_HEADER_CHECK(0x80, 0, 0, 9, 0x40, 0xE0)

        stepinfo(">>>>>>>>>> Test CCID Busy - Xfer done\n");
        /* Response.  */
#if defined(UX_DEVICE_STANDALONE)
        slot_tester[0].state = SLOT_TESTER_STATE_NEXT;
#else
        tx_semaphore_put(&slot_tester[0].semaphore);
#endif
        status = _ccid_message_bulk_out_in(UX_NULL, 10, host_bulk_in, & host_bulk_in_length);
        UX_TEST_ASSERT(status == UX_SUCCESS);
        _TEST_CCID_BULK_IN_HEADER_CHECK(0x80, 64-10, 0, 8, 0, 0)
    }
#if UX_DEVICE_CLASS_CCID_MAX_N_SLOTS > 1
    /* Max number of slots exceed, response busy.  */
    {
        /* _XfrBlock(0) ... blocked.  */
        host_bulk_out[5] = 0; /* bSlot.  */
        host_bulk_out[6] = 8; /* bSeq.   */
        ...
        /* _XfrBlock(1) ... blocked.  */
        host_bulk_out[5] = 1; /* bSlot.  */
        host_bulk_out[6] = 2; /* bSeq.   */
        device_ccid_callback_log.handle = UX_NULL;
        status = _ccid_message_bulk_out_in(host_bulk_out, 10, UX_NULL, &host_bulk_in_length);
        _TEST_CCID_BULK_OUT_CHECK(ux_test_ccid_xfr_block, 10, host_bulk_out)

        /* _XfrBlock(2) ... response busy.  */
        host_bulk_out[5] = 2; /* bSlot.  */
        host_bulk_out[6] = 7; /* bSeq.   */
        device_ccid_callback_log.handle = UX_NULL;
        status = _ccid_message_bulk_out_in(host_bulk_out, 10, host_bulk_in, &host_bulk_in_length);
        _TEST_CCID_BULK_OUT_CHECK(UX_NULL, 10, host_bulk_out)
        _TEST_CCID_BULK_IN_HEADER_CHECK(0x80, 0, 2, 7, 0x40, 0xE0)

        /* Response(1).  */
        tx_semaphore_put(&slot_tester[1].semaphore);
        status = _ccid_message_bulk_out_in(UX_NULL, 10, host_bulk_in, & host_bulk_in_length);
        UX_TEST_ASSERT(status == UX_SUCCESS);
        _TEST_CCID_BULK_IN_HEADER_CHECK(0x80, 64-10, 1, 2, 0, 0)
        ...
        /* Response(2).  */
        tx_semaphore_put(&slot_tester[2].semaphore);
        status = _ccid_message_bulk_out_in(UX_NULL, 10, host_bulk_in, & host_bulk_in_length);
        UX_TEST_ASSERT(status == UX_SUCCESS);
        _TEST_CCID_BULK_IN_HEADER_CHECK(0x80, 64-10, 2, 7, 0, 0)
    }
#endif
...
{
UINT        status;
    /* PC_to_RDR_XfrBlock, RDR_to_PC_DataBlock.  */tx_thread_sleep(10);
    _ux_utility_memory_set(host_bulk_out, 0, 10);
    _ux_utility_memory_set(host_bulk_out_busy, 0, 10);
    /* Same slot response busy.  */
    {
        /* _XfrBlock ... blocked.  */
        stepinfo(">>>>>>>>>> Test CCID Abort - Xfer blocked\n");
        host_bulk_out[0] = 0x6F;
        host_bulk_out[5] = 0; /* bSlot.  */
        host_bulk_out[6] = 8; /* bSeq.   */
        _ux_utility_long_put(host_bulk_out+1, 0);
        ...
        /* _XfrBlock ... response busy.  */
        stepinfo(">>>>>>>>>> Test CCID Abort - Xfer busy\n");
        host_bulk_out_busy[0] = 0x6F;
        host_bulk_out_busy[5] = 0; /* bSlot.  */
        host_bulk_out_busy[6] = 9; /* bSeq.   */
        device_ccid_callback_log.handle = UX_NULL;
        status = _ccid_message_bulk_out_in(host_bulk_out_busy, 10, host_bulk_in, &host_bulk_in_length);
#if defined(UX_DEVICE_STANDALONE)
        _TEST_CCID_BULK_OUT_CHECK(ux_test_ccid_xfr_block, 10, host_bulk_out) /* Still polled in wait state.  */
#else
        _TEST_CCID_BULK_OUT_CHECK(UX_NULL, 10, host_bulk_out)
#endif
        _TEST_CCID_BULK_IN_HEADER_CHECK(0x80, 0, 0, 9, 0x40, 0xE0)
        ...
        /* Abort.  */
        stepinfo(">>>>>>>>>> Test CCID Abort - Xfer abort\n");
        status = _ccid_abort(0, 12);
        UX_TEST_ASSERT(status == UX_SUCCESS);

        /* _XfrBlock ... OK.  */
        stepinfo(">>>>>>>>>> Test CCID Abort - Xfer OK\n");
        host_bulk_out[0] = 0x6F;
        host_bulk_out[5] = 0; /* bSlot.  */
        ...
        status = _ccid_message_bulk_out_in(host_bulk_out, 10, host_bulk_in, &host_bulk_in_length);
        _TEST_CCID_BULK_OUT_CHECK(ux_test_ccid_xfr_block, 10, host_bulk_out)
        _TEST_CCID_BULK_IN_HEADER_CHECK(0x80, 64-10, 0, 20, 0, 0)
    }
}
...
UCHAR                                               *parameter_u8 = (UCHAR*)parameter_u32;
    ...
    _ccid_invalid_slot_test();
    _ccid_busy_slot_test();
    _ccid_abort_test();
...
#if defined(UX_DEVICE_STANDALONE)
    switch(slot_tester[cmd->bSlot].state)
    {
    case SLOT_TESTER_STATE_START:
        slot_tester[cmd->bSlot].state = SLOT_TESTER_STATE_WAIT;
        return(UX_STATE_WAIT);
    case SLOT_TESTER_STATE_WAIT:
        return(UX_STATE_WAIT);
    case SLOT_TESTER_STATE_NEXT:
        slot_tester[cmd->bSlot].state = SLOT_TESTER_STATE_IDLE;
        break;
    default:
        slot_tester[cmd->bSlot].state = SLOT_TESTER_STATE_START;
        return(UX_STATE_WAIT);
    }
#else

    /* Semaphore wait.  */
    tx_semaphore_get(&slot_tester[cmd->bSlot].semaphore, slot_tester[cmd->bSlot].timeout);
    /* Abort?  */
    if (slot_tester[cmd->bSlot].flags & SLOT_TESTER_FLAG_ABORT)
    {
        slot_tester[cmd->bSlot].flags &= ~SLOT_TESTER_FLAG_ABORT;
        return(UX_ERROR);
    }
#endif
...
UX_DEVICE_CLASS_CCID_RDR_TO_PC_SLOT_STATUS_HEADER           *rsp;
    ...
    if (io_msg == UX_NULL)
    {
        ...
        slot_tester[slot].flags |= SLOT_TESTER_FLAG_ABORT;
#if defined(UX_DEVICE_STANDALONE)
        slot_tester[slot].state = SLOT_TESTER_STATE_NEXT;
        return(UX_STATE_NEXT);
#else
        tx_semaphore_put(&slot_tester[slot].semaphore);
        return(UX_SUCCESS);
#endif
    }
    /* Bulk OUT Abort.  */
