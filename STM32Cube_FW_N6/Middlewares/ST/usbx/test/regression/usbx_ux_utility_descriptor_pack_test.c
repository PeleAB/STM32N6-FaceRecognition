...
#else
void    usbx_ux_utility_descriptor_pack_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR                            *memory_pointer;
    ...
    /* Inform user.  */
    printf("Running ux_utility_descriptor_pack Test............................. ");
...
struct _input {
    ...
    UCHAR  _align_f5[2];
    ULONG f5;
} input = {0x03020100, 0x0504, 0x6, {0}, 0x0807, {0,0}, 0x0c0b0a09};
    _ux_utility_memory_set(output, 0xFF, sizeof(output));
    _ux_utility_descriptor_pack((UCHAR *)&input, structure, 5, output);
#else
ULONG input[]     = {0x03020100, 0x0504, 0x6, 0x0807, 0x0c0b0a09, 0x0};
    ...
    for (i = 0; i <= 0x0c; i ++)
    {
        if (output[i] != i)
        {
            printf("ERROR #%d: expect 0x%lx but got 0x%x\n", __LINE__, i, output[i]);
            error_counter ++;
        }
    }
