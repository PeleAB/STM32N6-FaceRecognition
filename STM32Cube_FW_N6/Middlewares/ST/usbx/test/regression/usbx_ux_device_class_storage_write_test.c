...
#else
void    usbx_ux_device_class_storage_write_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR *                          memory_pointer;
    ...
    /* Inform user.  */
    printf("Running ux_device_class_storage_write Test.......................... ");
    stepinfo("\n");
...
UX_SLAVE_CLASS_STORAGE                      *slave_storage;
    ...
    stepinfo(">>>>>>>>>>>>>>> UX_SLAVE_CLASS_STORAGE_SCSI_WRITE16 - success\n");
    ...
    _test_init_cbw_WRITE(UX_SLAVE_CLASS_STORAGE_SCSI_READ16, 0, 1);
    ...
    status = _test_transfer_data(buffer, 1024, UX_FALSE);
    if (status != UX_TRANSFER_STALLED)
    {
        printf("ERROR #%d: code 0x%x\n", __LINE__, status);
        test_control_return(1);
