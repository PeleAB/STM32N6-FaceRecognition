...
#else
void    usbx_ux_host_stack_new_device_create_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR                            *memory_pointer;
    ...
    if (status != UX_SUCCESS)
    {
        ...
        printf("Running ux_host_stack_new_device_create Test........................ ERROR #1\n");
        test_control_return(1);
    }
    ...
    if (status != UX_SUCCESS)
    {
        ...
        printf("Running ux_host_stack_new_device_create Test........................ ERROR #2\n");
        test_control_return(1);
    }
    ...
    if (status != UX_SUCCESS)
    {
        ...
        printf("Running ux_host_stack_new_device_create Test........................ ERROR #3\n");
        test_control_return(1);
    }
    ...
    if (status != UX_SUCCESS)
    {
        ...
        printf("Running ux_host_stack_new_device_create Test........................ ERROR #5\n");
        test_control_return(1);
    }
    ...
    if (status != UX_SUCCESS)
    {
        ...
        printf("Running ux_host_stack_new_device_create Test........................ ERROR #6\n");
        test_control_return(1);
    }
    ...
    if (status != UX_SUCCESS)
    {
        ...
        printf("Running ux_host_stack_new_device_create Test........................ ERROR #7\n");
        test_control_return(1);
    }
    ...
    if (status != UX_SUCCESS)
    {
        ...
        printf("Running ux_host_stack_new_device_create Test........................ ERROR #4\n");
        test_control_return(1);
    }
    ...
    if (status != TX_SUCCESS)
    {
        ...
        printf("Running ux_host_stack_new_device_create Test........................ ERROR #8\n");
        test_control_return(1);
    }
    ...
    if (status != TX_SUCCESS)
    {
        ...
        printf("Running ux_host_stack_new_device_create Test........................ ERROR #9\n");
        test_control_return(1);
    }
}
...
UINT             i;
    ...
    /* Inform user.  */
    printf("Running ux_host_stack_new_device_create Test........................ ");
    ...
    /* Simulate device creation errors. */

    test_error_cases = UX_TRUE;

    hcd = &_ux_system_host -> ux_system_host_hcd_array[0];

    /* Modify number of devices to generate error. */
    nb_devices = hcd->ux_hcd_nb_devices;
    hcd -> ux_hcd_nb_devices = UX_MAX_USB_DEVICES + 1;

    status = _ux_host_stack_new_device_create(hcd, UX_NULL, 0, UX_FULL_SPEED_DEVICE, 50, &created_device);
    if (status != UX_TOO_MANY_DEVICES)
    {

        printf("ERROR #%d: expect error for too many devices\n", __LINE__);
        error_counter ++;
    }
    /* Restore correct number of devices. */
    hcd -> ux_hcd_nb_devices = nb_devices;

    /* Get new devices until error. */
    for (i = 0; i < UX_MAX_USB_DEVICES; i ++)
    {

        device = _ux_host_stack_new_device_get();

        /* If we are not able to get new device, it's OK. */
        if (device == UX_NULL)
            break;
    }

    /* Expect UX_TOO_MANY_DEVICES error */
    status = _ux_host_stack_new_device_create(hcd, UX_NULL, 0, UX_FULL_SPEED_DEVICE, 50, &created_device);
    if (status != UX_TOO_MANY_DEVICES)
    {
    ...
    if (hcd->ux_hcd_nb_devices != nb_devices)
    {

        printf("ERROR #%d: number of devices should not be changed if get new device fail\n", __LINE__);
        error_counter ++;
    }
