...
#else
void    usbx_ux_host_class_storage_max_lun_get_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR *                          memory_pointer;
    ...
    /* Inform user.  */
    printf("Running ux_host_class_storage_max_lun_get Test...................... ");
    stepinfo("\n");
...
UX_DEVICE                                   *device;
    ...
    stepinfo(">>>>>>>>>>>>>>> ux_host_class_storage_max_lun_get - STALLed\n");
    buffer[0] = 2;
    ux_test_hcd_sim_host_set_actions(stall_GetMaxLun);
    ...
    if (storage->ux_host_class_storage_max_lun != 0)
    {
        printf("ERROR #%d\n", __LINE__);
        test_control_return(1);
    }

    stepinfo(">>>>>>>>>>>>>>> ux_host_class_storage_max_lun_get - return length error\n");
    buffer[0] = 2;
    ux_test_hcd_sim_host_set_actions(length_error_GetMaxLun);
    status = _ux_host_class_storage_max_lun_get(storage);
    if (status != UX_SUCCESS)
    {
        printf("ERROR #%d, code 0x%x\n", __LINE__, status);
    ...
    stepinfo(">>>>>>>>>>>>>>> ux_host_class_storage_max_lun_get - over UX_MAX_HOST_LUN - 1\n");
    buffer[0] = UX_MAX_HOST_LUN;
    ux_test_hcd_sim_host_set_actions(replaced_GetMaxLun);
