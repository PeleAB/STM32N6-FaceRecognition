...
static UCHAR hid_report_descriptor[] = {
    ...
    0x0b, 0x31, 0x00, 0x04, 0x00,  //   USAGE (Sports:Slope)
...
#else
void    usbx_hid_report_descriptor_extended_usage_test_application_define(void *first_unused_memory)
#endif
{
...
UINT                            descriptor_size = HID_REPORT_LENGTH;
    ...
    /* Inform user.  */
    printf("Running HID Report Descriptor Extended Usages Test....................... ");
...
UX_HOST_CLASS_HID_REPORT            *input_report_descriptor;
    ...
    status = _ux_host_class_hid_report_decompress(hid, &input_report_request, report_buffer_compressed, 1);
    if (status != UX_SUCCESS)
    {
    ...
    if (
        report_buffer_decompressed[0] != 0x00040031
        )
    {
