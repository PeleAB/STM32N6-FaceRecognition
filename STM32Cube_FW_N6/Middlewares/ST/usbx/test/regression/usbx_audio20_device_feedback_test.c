...
#else
    slave_audio_stream_parameter[0].ux_device_class_audio_stream_parameter_task_function = ux_device_class_audio_write_task_function;
#endif

#if defined(UX_DEVICE_CLASS_AUDIO_FEEDBACK_SUPPORT)
#if !defined(UX_DEVICE_STANDALONE)
    slave_audio_stream_parameter[0].ux_device_class_audio_stream_parameter_feedback_thread_entry = ux_device_class_audio_feedback_thread_entry;
#else
    slave_audio_stream_parameter[0].ux_device_class_audio_stream_parameter_feedback_task_function = _ux_device_class_audio_feedback_task_function;
#endif
#endif
...
#else
    slave_audio_stream_parameter[1].ux_device_class_audio_stream_parameter_task_function = ux_device_class_audio_read_task_function;
#endif
#if defined(UX_DEVICE_CLASS_AUDIO_FEEDBACK_SUPPORT)
#if !defined(UX_DEVICE_STANDALONE)
...
#else
    slave_audio_stream_parameter[1].ux_device_class_audio_stream_parameter_feedback_task_function = _ux_device_class_audio_feedback_task_function;
#endif
#endif
...
ULONG                                               temp;
    ...
    _ux_utility_memory_set(test_tmp, 0, 16);
    _ux_utility_memory_copy(test_tmp, "test", 4);

    /* 8 frame should be available.  */
    for (test_n = 0; test_n < 8; test_n ++)
    {
        // printf("%ld: %ld: %s\n", test_n, buffer_log[test_n].length, buffer_log[test_n].data);
        test_tmp[4] = (UCHAR)(test_n + '0');

        UX_TEST_ASSERT(buffer_log[test_n].length == 16);

        status = _ux_utility_memory_compare(buffer_log[test_n].data, test_tmp, 6);
        UX_TEST_CHECK_SUCCESS(status);
    }
    ...
    for (test_tx_ins_way = 0; test_tx_ins_way < 2; test_tx_ins_way ++)
    {
        ...
        /* Prepare data for checking.  */
        _ux_utility_memory_set(test_tmp, 0, 32);
        ...
        _ux_utility_memory_copy(test_tmp, "insert", 6);
        for (; test_n < 14; test_n ++)
        {
            // printf("%ld: %ld: %s\n", test_n, buffer_log[test_n].length, buffer_log[test_n].data);
            test_tmp[6] = (UCHAR)(((10 - (test_n - 4)) % 26) + 'A');

            UX_TEST_ASSERT(buffer_log[test_n].length == 32);

            status = _ux_utility_memory_compare(buffer_log[test_n].data, test_tmp, 8);
            UX_TEST_CHECK_SUCCESS(status);
        }
        /* Then under-run, 0 length packets.  */
        for(;test_n < buffer_log_count; test_n ++)
    ...
    for (test_n = 0; test_n < 10; test_n ++)
    {
        test_tmp[0] = (UCHAR)(test_n + '0');
        slave_audio_rx_simulate_one_frame(test_tmp, 1);
    }
    ...
    for (test_n = 0; test_n < 4; test_n ++)
    {
        ...
        UX_TEST_CHECK_SUCCESS(status);
        // printf("%lx\n", temp);
        test_tmp[0] = (UCHAR)(test_n * 2 + '0');
        test_tmp[1] = (UCHAR)(test_n * 2 + '1');
        UX_TEST_CHECK_SUCCESS(_ux_utility_memory_compare(test_tmp, &temp, 2));
    }
    ...
    for (test_n = 0; test_n < 3; test_n ++)
    {
        ...
        UX_TEST_CHECK_SUCCESS(status);
        // printf("%lx\n", temp);
        test_tmp[0] = (UCHAR)(test_n * 3 + '0');
        test_tmp[1] = (UCHAR)(test_n * 3 + '1');
        test_tmp[2] = (UCHAR)(test_n * 3 + '2');
        UX_TEST_CHECK_SUCCESS(_ux_utility_memory_compare(test_tmp, &temp, 3));
    }
    ...
    for (test_n = 0; test_n < 3; test_n ++)
    {
        ...
        UX_TEST_CHECK_SUCCESS(status);
        // printf("%lx\n", temp);
        test_tmp[0] = ((test_n * 4 + 0) % 10) + '0';
        test_tmp[1] = ((test_n * 4 + 1) % 10) + '0';
        test_tmp[2] = ((test_n * 4 + 2) % 10) + '0';
        test_tmp[3] = ((test_n * 4 + 3) % 10) + '0';
        UX_TEST_CHECK_SUCCESS(_ux_utility_memory_compare(test_tmp, &temp, 4));
    }
