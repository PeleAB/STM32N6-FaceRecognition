...
static TX_THREAD                           tx_test_thread_hid_read;
static TX_THREAD                           tx_test_thread_hid_write;
static TX_SEMAPHORE                        tx_test_semaphore_hid_trigger;
static VOID                                tx_test_hid_read_entry(ULONG);
static VOID                                tx_test_hid_write_entry(ULONG);
...
static UX_HOST_CLASS_HID            *host_hid = UX_NULL;
...
static UX_SLAVE_CLASS_HID              *device_hid = UX_NULL;
static UX_SLAVE_CLASS_HID_PARAMETER    device_hid_parameter;
...
static unsigned char device_framework_full_speed[] = {
    ...
    */
    ...
    _INTERFACE_DESCRIPTOR(0, 0, 1, 0x03, 0x00, 0x00)
    /* HID descriptor */
    0x09, 0x21, 0x10, 0x01, 0x21, 0x01, 0x22, _W0(HID_KEYBOARD_REPORT_LENGTH), _W1(HID_KEYBOARD_REPORT_LENGTH),
    _ENDPOINT_DESCRIPTOR(0x81, 0x03, 8, 0x0A)
};
...
static UX_TEST_SETUP _HidReportDescriptorGet = {0x81,0x06,0x2200,0x0000};
...
static VOID ux_test_on_hid_get_report_descriptor(UX_TEST_ACTION *action, VOID *params)
{
/* Test is hid instance allocated.  */
...
UX_HOST_CLASS_HID       *hid;
    ...
    hid = (UX_HOST_CLASS_HID *)ifc->ux_interface_class_instance;
    UX_TEST_ASSERT(hid != UX_NULL);
    UX_TEST_ASSERT(hid->ux_host_class_hid_allocated != UX_NULL);
    /* Memory allocated, disconnect device to see if memory is double freed.  */
    _test_simulate_disconnect = 1;
}
static UX_TEST_HCD_SIM_ACTION call_on_HidReportDescriptorGet[] = {
/* function, request to match,
   port action, port status,
   request action, request EP, request data, request actual length, request status,
   status, additional callback,
   no_return */
{   UX_HCD_TRANSFER_REQUEST, &_HidReportDescriptorGet,
        UX_FALSE, UX_TEST_PORT_STATUS_DISC,
        UX_TEST_SETUP_MATCH_REQ, 0, UX_NULL, 0, 0,
        UX_SUCCESS, ux_test_on_hid_get_report_descriptor,
        UX_TRUE}, /* Invoke callback & continue */
{   0   }
};
...
UX_HOST_CLASS_HID *hid_inst = (UX_HOST_CLASS_HID *) inst;
...
static VOID    test_hid_instance_activate(VOID *dummy_instance)
{
    if (device_hid == UX_NULL)
        device_hid = (UX_SLAVE_CLASS_HID *)dummy_instance;
}
static VOID    test_hid_instance_deactivate(VOID *dummy_instance)
{
    if ((VOID*)device_hid == dummy_instance)
        device_hid = UX_NULL;
}
...
#else
void usbx_msrc_81572_standalone_host_hid_allocated_enum_free_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR *                  memory_pointer;
    ...
    printf("Running MSRC 81574 - Standalone Host HID Alloc Enum Free Test....... ");
#if !defined(UX_HOST_STANDALONE)
    printf("Skip\n");
    ...
    _ux_utility_memory_set(&device_hid_parameter, 0, sizeof(device_hid_parameter));
    device_hid_parameter.ux_device_class_hid_parameter_report_address = hid_keyboard_report;
    device_hid_parameter.ux_device_class_hid_parameter_report_length  = HID_KEYBOARD_REPORT_LENGTH;
    device_hid_parameter.ux_slave_class_hid_instance_activate   = test_hid_instance_activate;
    device_hid_parameter.ux_slave_class_hid_instance_deactivate = test_hid_instance_deactivate;
    /* Initialize the device cdc class. This class owns both interfaces starting with 0. */
    status  = ux_device_stack_class_register(_ux_system_slave_class_hid_name,
                                             ux_device_class_hid_entry,
                                             1, 0, &device_hid_parameter);
    ...
    ux_test_hcd_sim_host_set_actions(call_on_HidReportDescriptorGet);
...
{
    if (device_hid == UX_NULL && host_hid == UX_NULL)
        return(UX_SUCCESS);
    return(UX_ERROR);
}
