...
static UINT                         device_stall_count = 9;

static UX_HOST_CLASS_GSER           *host_gser = UX_NULL;
...
#define _CFG_TOTAL_LEN (9+(9+7+7)*2 + (9+7))
...
static unsigned char device_framework_full_speed[] = {
    ...
    */
    ...
    /* REF: CFG (IFC BO BI) * 3 */
    _CONFIGURATION_DESCRIPTOR(_CFG_TOTAL_LEN, 3, 1)
    _INTERFACE_DESCRIPTOR(0, 0, 2, 0xFF, 0xFF, 0x00)
    _ENDPOINT_DESCRIPTOR(0x81, 0x02, 64, 0x00)
    _ENDPOINT_DESCRIPTOR(0x02, 0x02, 64, 0x00)
    _INTERFACE_DESCRIPTOR(1, 0, 2, 0xFF, 0xFF, 0x00)
    _ENDPOINT_DESCRIPTOR(0x83, 0x02, 64, 0x00)
    _ENDPOINT_DESCRIPTOR(0x04, 0x02, 64, 0x00)
    _INTERFACE_DESCRIPTOR(2, 0, 1, 0xFF, 0xFF, 0x00)
    _ENDPOINT_DESCRIPTOR(0x85, 0x02, 64, 0x00)
};
...
UX_HOST_CLASS_GSER *gser_inst = (UX_HOST_CLASS_GSER *) inst;
...
#else
void    usbx_msrc_81232_host_gser_inst_free_tests_application_define(void *first_unused_memory)
#endif
{
...
CHAR *                  memory_pointer;
    ...
    printf("Running MSRC 81232 - Host GSER inst free Test....................... ");
    ...
    status =  ux_host_stack_class_register(_ux_system_host_class_gser_name, _ux_host_class_gser_entry);
...
{
    if (device_dummy == UX_NULL && host_gser == UX_NULL)
        return(UX_SUCCESS);
    return(UX_ERROR);
}
