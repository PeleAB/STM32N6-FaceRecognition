...
static VOID action_disconnect_on_stall(struct UX_TEST_ACTION_STRUCT *action, VOID *params)
{
...
static UX_TEST_HCD_SIM_ACTION disconnect_on_stall[] = {
/* function, request to match,
   port action, port status,
   request action, request EP, request data, request actual length, request status,
   status, additional callback,
   no_return */
{   UX_DCD_STALL_ENDPOINT, UX_NULL,
        UX_FALSE, UX_TEST_PORT_STATUS_DISC,
        UX_TEST_MATCH_EP, 0x81, UX_NULL, 0, UX_SUCCESS,
        UX_SUCCESS, action_disconnect_on_stall,
        UX_FALSE}, /* Invoke callback & no continue */
{   0   }
};
...
#else
void    usbx_ux_device_class_storage_thread_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR *                          memory_pointer;
    ...
    /* Inform user.  */
    printf("Running ux_device_class_storage_thread Test......................... ");
    stepinfo("\n");
...
static void _test_init_cbw(UCHAR dCBWSignature_error,
    ULONG dCBWDataTransferLength, UCHAR dir, UCHAR lun, UCHAR bCBWCBLength,
    UCHAR cb_op)
{
...
UCHAR               *cbw;
    ...
    cbw =  (UCHAR *) storage -> ux_host_class_storage_cbw;
    _ux_host_class_storage_cbw_initialize(storage, dir ? 0x80 : 0, dCBWDataTransferLength, bCBWCBLength);

    if (dCBWSignature_error)
        *(cbw + UX_HOST_CLASS_STORAGE_CBW_SIGNATURE)    = 0x00;

    *(cbw + UX_HOST_CLASS_STORAGE_CBW_LUN)              = lun;

    *(cbw + UX_HOST_CLASS_STORAGE_CBW_CB + 0)           = cb_op;
}

static UINT _test_send_cbw(UCHAR length_error)
{
...
UCHAR           *cbw;
    ...
    transfer_request -> ux_transfer_request_requested_length =  UX_HOST_CLASS_STORAGE_CBW_LENGTH - length_error;
...
UINT            command_length = UX_HOST_CLASS_STORAGE_REQUEST_SENSE_COMMAND_LENGTH_SBC;
    ...
    *(cbw + UX_HOST_CLASS_STORAGE_CBW_CB + UX_HOST_CLASS_STORAGE_REQUEST_SENSE_ALLOCATION_LENGTH) =  UX_HOST_CLASS_STORAGE_REQUEST_SENSE_RESPONSE_LENGTH;
    ...
    status = _test_send_cbw(UX_FALSE);
    if (status == UX_SUCCESS)
    {
...
UX_SLAVE_CLASS_STORAGE                      *slave_storage;
    ...
    stepinfo(">>>>>>>>>>>>>>> ux_device_class_storage_thread - command unknown (write)\n");
    _test_init_cbw(UX_FALSE, 10,  UX_FALSE, 0, 6, 0xFF);
    status = _test_send_cbw(UX_FALSE);
    if (status != UX_SUCCESS)
    {
    ...
    status = _test_transfer_data(buffer, 512, UX_FALSE);
    ...
    stepinfo(">>>>>>>>>>>>>>> ux_device_class_storage_thread - command unknown (read)\n");
    _test_init_cbw(UX_FALSE, 10,  UX_TRUE, 0, 6, 0xFF);
    status = _test_send_cbw(UX_FALSE);
    if (status != UX_SUCCESS)
    {
        printf("ERROR #%d: code 0x%x\n", __LINE__, status);
    ...
    status = _test_transfer_data(buffer, 512, UX_TRUE);
    if (status != UX_TRANSFER_STALLED)
    {
        printf("ERROR #%d: code 0x%x\n", __LINE__, status);
        test_control_return(1);
    ...
    stepinfo(">>>>>>>>>>>>>>> ux_device_class_storage_thread - command unknown (no data)\n");
    _test_init_cbw(UX_FALSE, 0,  UX_TRUE, 0, 6, 0xFF);
    status = _test_send_cbw(UX_FALSE);
    if (status != UX_SUCCESS)
    {
        printf("ERROR #%d: code 0x%x\n", __LINE__, status);
    ...
    stepinfo(">>>>>>>>>>>>>>> ux_device_class_storage_thread - bad LUN\n");
    _test_init_cbw(UX_FALSE, 0,  UX_TRUE, (UCHAR)slave_storage->ux_slave_class_storage_number_lun, UX_HOST_CLASS_STORAGE_TEST_READY_COMMAND_LENGTH_SBC, UX_HOST_CLASS_STORAGE_SCSI_TEST_READY);
    status = _test_send_cbw(UX_FALSE);
    if (status != UX_SUCCESS)
    {
        printf("ERROR #%d: code 0x%x\n", __LINE__, status);
    ...
    stepinfo(">>>>>>>>>>>>>>> ux_device_class_storage_thread - bad received length\n");
    _test_init_cbw(UX_FALSE, 0,  UX_TRUE, 0, UX_HOST_CLASS_STORAGE_TEST_READY_COMMAND_LENGTH_SBC, UX_HOST_CLASS_STORAGE_SCSI_TEST_READY);
    status = _test_send_cbw(UX_TRUE);
    if (status != UX_SUCCESS)
    {
        printf("ERROR #%d: code 0x%x\n", __LINE__, status);
        test_control_return(1);
    ...
    stepinfo(">>>>>>>>>>>>>>> ux_device_class_storage_thread - bad signature\n");
    _test_init_cbw(UX_TRUE, 0,  UX_TRUE, 0, UX_HOST_CLASS_STORAGE_TEST_READY_COMMAND_LENGTH_SBC, UX_HOST_CLASS_STORAGE_SCSI_TEST_READY);
    status = _test_send_cbw(UX_FALSE);
    if (status != UX_SUCCESS)
    {
        printf("ERROR #%d: code 0x%x\n", __LINE__, status);
    ...
    stepinfo(">>>>>>>>>>>>>>> ux_device_class_storage_thread - bad CBW length\n");
    _test_init_cbw(UX_FALSE, 0,  UX_TRUE, 0, 0, UX_HOST_CLASS_STORAGE_SCSI_TEST_READY);
    status = _test_send_cbw(UX_FALSE);
    if (status != UX_SUCCESS)
    {
        printf("ERROR #%d: code 0x%x\n", __LINE__, status);
    ...
    stepinfo(">>>>>>>>>>>>>>> ux_device_class_storage_thread - CSW transfer error after stall\n");
    _test_init_cbw(UX_FALSE, 0,  UX_TRUE, 0, 6, 0xFF);
    ...
    {
        printf("ERROR #%d: error callback not detected\n", __LINE__);
        test_control_return(1);
    }
    status = _test_wait_csw();
    if (status == UX_SUCCESS)
    ...
    stepinfo(">>>>>>>>>>>>>>> ux_device_class_storage_thread - disconnect after stall\n");
    _test_init_cbw(UX_FALSE, 0,  UX_TRUE, 0, 6, 0xFF);
    ux_test_dcd_sim_slave_set_actions(disconnect_on_stall);
    status = _test_send_cbw(UX_FALSE);
    if (status != UX_SUCCESS)
    {
        printf("ERROR #%d: code 0x%x\n", __LINE__, status);
    ...
    /* Restore.  */
    slave_device->ux_slave_device_state = UX_DEVICE_CONFIGURED;
