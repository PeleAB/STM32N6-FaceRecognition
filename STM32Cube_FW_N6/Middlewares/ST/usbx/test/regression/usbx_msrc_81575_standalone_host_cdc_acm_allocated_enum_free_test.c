...
static UX_TEST_SETUP _CDCACMLineCodingSet = {0x21,0x20,0x0000,0x0001};
...
static VOID ux_test_on_cdc_acm_set_line_coding(UX_TEST_ACTION *action, VOID *params)
{
/* Test is cdc_acm instance allocated.  */
...
UX_HOST_CLASS_CDC_ACM   *cdc_acm;
    ...
    ifc1 = ifc->ux_interface_next_interface;
    cdc_acm = (UX_HOST_CLASS_CDC_ACM *)ifc1->ux_interface_class_instance;
    UX_TEST_ASSERT(cdc_acm != UX_NULL);
    UX_TEST_ASSERT(cdc_acm->ux_host_class_cdc_acm_allocated != UX_NULL);
    /* Memory allocated, disconnect device to see if memory is double freed.  */
    _test_simulate_disconnect = 1;
}
static UX_TEST_HCD_SIM_ACTION call_on_CDCACMLineCodingSet[] = {
/* function, request to match,
   port action, port status,
   request action, request EP, request data, request actual length, request status,
   status, additional callback,
   no_return */
{   UX_HCD_TRANSFER_REQUEST, &_CDCACMLineCodingSet,
        UX_FALSE, UX_TEST_PORT_STATUS_DISC,
        UX_TEST_SETUP_MATCH_REQ, 0, UX_NULL, 0, 0,
        UX_SUCCESS, ux_test_on_cdc_acm_set_line_coding,
        UX_TRUE}, /* Invoke callback & continue */
{   0   }
};
...
#else
void usbx_msrc_81572_standalone_host_cdc_acm_allocated_enum_free_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR *                  memory_pointer;
    ...
    printf("Running MSRC 81575 - Standalone Host CDC ACM Alloc Enum Free Test... ");
#if !defined(UX_HOST_STANDALONE)
    printf("Skip\n");
    ...
    ux_test_hcd_sim_host_set_actions(call_on_CDCACMLineCodingSet);
