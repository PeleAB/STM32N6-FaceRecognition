...
static UX_SLAVE_CLASS_CDC_ACM_CALLBACK_PARAMETER    cdc_acm_slave_callback;
...
#else
void    usbx_standalone_device_cdc_acm_transmission_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR *                  memory_pointer;
    ...
    /* Inform user.  */
    printf("Running STANDALONE CDC ACM Transmission Test........................ ");
...
static void test_cdc_acm_device_transmission(void)
{
UINT        status;
ULONG       actual_length;
UCHAR       test_chr;
...
struct {
    ...
    for (test = 0; test < N_TEST; test ++)
    {
        ...
        tx_thread_sleep(test_length/64 + 10);
...
UINT                                                status;
    ...
    test_cdc_acm_device_transmission();
...
#define TEST_N_BUFFER                    64
static UCHAR test_cdc_acm_device_fifo[TEST_N_BUFFER][64];
static ULONG test_cdc_acm_device_fifo_length[TEST_N_BUFFER];
static ULONG test_cdc_acm_device_fifo_read_count;
static ULONG test_cdc_acm_device_fifo_write_count;
static UINT  test_cdc_acm_device_fifo_read_status;
static UINT  test_cdc_acm_device_fifo_write_status;
static UINT test_cdc_acm_device_write_callback(
    struct UX_SLAVE_CLASS_CDC_ACM_STRUCT *cdc_acm,
    UINT status, ULONG length)
{
UINT   fifo_index;
    test_cdc_acm_device_fifo_write_status = status;
    test_cdc_acm_device_fifo_write_count ++;
    if (test_cdc_acm_device_fifo_write_count >= test_cdc_acm_device_fifo_read_count)
        return(0);

    fifo_index = test_cdc_acm_device_fifo_write_count % TEST_N_BUFFER;
    status = ux_device_class_cdc_acm_write_with_callback(cdc_acm_slave,
                                test_cdc_acm_device_fifo[fifo_index],
                                test_cdc_acm_device_fifo_length[fifo_index]);
    if (status != UX_SUCCESS)
    {
        printf("ERROR #%d write fail %x\n", __LINE__, status);
        test_control_return(1);
    }
    return(0);
}
static UINT test_cdc_acm_device_read_callback(
    struct UX_SLAVE_CLASS_CDC_ACM_STRUCT *cdc_acm,
    UINT status, UCHAR *data_pointer, ULONG length)
{
UINT   fifo_index = test_cdc_acm_device_fifo_read_count % TEST_N_BUFFER;
UINT   start = test_cdc_acm_device_fifo_read_count == test_cdc_acm_device_fifo_write_count;
    test_cdc_acm_device_fifo_read_status = status;
    if (status != UX_SUCCESS)
        return(0);
    if (test_cdc_acm_device_fifo_read_count - test_cdc_acm_device_fifo_write_count > TEST_N_BUFFER)
    {
        printf("ERROR #%d fifo (%d) overflow\n", __LINE__, TEST_N_BUFFER);
        return(0);
    }

    test_cdc_acm_device_fifo_read_count ++;
    ux_utility_memory_copy(test_cdc_acm_device_fifo[fifo_index], data_pointer, length);
    test_cdc_acm_device_fifo_length[fifo_index] = length;
    if (!start)
        return(0);

    /* Try write.  */
    fifo_index = test_cdc_acm_device_fifo_write_count % TEST_N_BUFFER;
    ux_device_class_cdc_acm_write_with_callback(cdc_acm_slave,
                                test_cdc_acm_device_fifo[fifo_index],
                                test_cdc_acm_device_fifo_length[fifo_index]);
    return(0);
}
...
UX_SLAVE_CLASS_CDC_ACM  *cdc_acm = UX_NULL;
    ...
    cdc_acm_slave_callback.ux_device_class_cdc_acm_parameter_read_callback = test_cdc_acm_device_read_callback;
    cdc_acm_slave_callback.ux_device_class_cdc_acm_parameter_write_callback = test_cdc_acm_device_write_callback;
    ...
    while(1)
    {
        ...
        if (cdc_acm != cdc_acm_slave && cdc_acm_slave)
        {
            cdc_acm = cdc_acm_slave;

            /* Start transmission.  */
            test_cdc_acm_device_fifo_read_count = 0;
            test_cdc_acm_device_fifo_write_count = 0;
            status = ux_device_class_cdc_acm_ioctl(cdc_acm_slave,
                        UX_SLAVE_CLASS_CDC_ACM_IOCTL_TRANSMISSION_START,
                        (VOID*)&cdc_acm_slave_callback);
            if (status != UX_SUCCESS)
            {
                printf("ERROR %d transmission start fail %x\n", __LINE__, status);
                test_control_return(1);
            }
        }
