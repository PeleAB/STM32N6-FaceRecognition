...
#else
void    usbx_ux_utility_physical_address_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR                            *memory_pointer;
    ...
    /* Inform user.  */
    printf("Running ux_utility_physical_address Test (and virtual).............. ");
...
#define TEST_ADDR ((void *)0x1234567)
    pointer = _ux_utility_physical_address(TEST_ADDR);
    if (pointer != TEST_ADDR)
    {
        printf("ERROR #%d: expected addr %p but got %p\n", __LINE__, TEST_ADDR, pointer);
        error_counter ++;
    }

    pointer = _ux_utility_virtual_address(TEST_ADDR);
    if (pointer != TEST_ADDR)
    {
        printf("ERROR #%d: expected addr %p but got %p\n", __LINE__, TEST_ADDR, pointer);
        error_counter ++;
