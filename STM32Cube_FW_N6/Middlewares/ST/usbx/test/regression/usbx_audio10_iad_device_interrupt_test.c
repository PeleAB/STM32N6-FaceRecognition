...
#define                             UX_TEST_STATUS_QUEUE_MAX_SIZE       8
...
/* 8  iTerminal                                               */ 0,
/* --------------- Audio 1.0 AC Interrupt Endpoint Descriptor */
...
CHAR *                  memory_pointer;
    ...
    /* Inform user.  */
    printf("Running Audio 1.0 IAD Device Interrupt Functionality Test........... ");
    stepinfo("\n");
...
#else
    slave_audio_stream_parameter[0].ux_device_class_audio_stream_parameter_task_function = _ux_device_class_audio_write_task_function;
#endif
...
#else
    slave_audio_stream_parameter[1].ux_device_class_audio_stream_parameter_task_function = _ux_device_class_audio_read_task_function;
#endif
    ...
    slave_audio_parameter.ux_device_class_audio_parameter_status_size = 2;                                      /* UAC 1.0.  */
    slave_audio_parameter.ux_device_class_audio_parameter_status_queue_size = UX_TEST_STATUS_QUEUE_MAX_SIZE;    /* Queue 8 interrupts.  */
#endif
...
#if defined(UX_DEVICE_CLASS_AUDIO_INTERRUPT_SUPPORT)
static void _test_device_uac_interrupts(ULONG queue_size)
{
ULONG           i, actual;
UINT            status;
UCHAR           status_word[2];
UCHAR           host_status[2];
    stepinfo("== Device UAC status queue size %ld\n", queue_size);
    /* Setup queue size.  */
    slave_audio->ux_device_class_audio_status_queue_bytes = queue_size * slave_audio->ux_device_class_audio_status_size;
    slave_audio->ux_device_class_audio_status_head = slave_audio->ux_device_class_audio_status_queue;
    slave_audio->ux_device_class_audio_status_tail = slave_audio->ux_device_class_audio_status_queue;

    /* Fill status up to size.  */
    for (i = 0; i < queue_size; i ++)
    {
        status_word[0] = i;
        status_word[1] = i;
        status = ux_device_class_audio_interrupt_send(slave_audio, status_word);
        UX_TEST_CHECK_SUCCESS(status);
    }
    ...
    status = ux_device_class_audio_interrupt_send(slave_audio, status_word);
    UX_TEST_CHECK_SUCCESS(status);
    /* Fill one more status, fail.  */
    status_word[0] = i;
    status_word[1] = i;
    status = ux_device_class_audio_interrupt_send(slave_audio, status_word);
    UX_TEST_CHECK_CODE(UX_BUSY, status);
    /* Read status on host side up to size.  */
    _ux_host_class_dummy_set_timeout(dummy_control, 0x83, 0, 10);
    for (i = 0; i < queue_size; i ++)
    {
        status = _ux_host_class_dummy_transfer(dummy_control, 0x83, 0, host_status, 2, &actual);
        UX_TEST_CHECK_SUCCESS(status);
        UX_TEST_ASSERT(actual == 2);
        UX_TEST_ASSERT(host_status[0] == i);
        UX_TEST_ASSERT(host_status[1] == i);
    }
    /* Read one more status, fail.  */
    status = _ux_host_class_dummy_transfer(dummy_control, 0x83, 0, host_status, 2, &actual);
    UX_TEST_CHECK_NOT_SUCCESS(status);
    /* Fill status and read status 2*queue_size times, success.  */
    ...
    /* The status queue is NULL.  */
    UX_TEST_ASSERT((slave_audio->ux_device_class_audio_status_head == slave_audio->ux_device_class_audio_status_tail) &&
                   (slave_audio->ux_device_class_audio_status_queued == 0));
}
#endif
...
UCHAR                                               test_tmp[32];
    ...
    /* Interrupt test.  */
#if defined(UX_DEVICE_CLASS_AUDIO_INTERRUPT_SUPPORT)
    for (test_n = 1; test_n <= UX_TEST_STATUS_QUEUE_MAX_SIZE; test_n <<= 1)
        _test_device_uac_interrupts(test_n);
#endif
