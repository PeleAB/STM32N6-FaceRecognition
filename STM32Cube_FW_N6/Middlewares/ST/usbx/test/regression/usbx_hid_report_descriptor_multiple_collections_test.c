...
#else
void    usbx_hid_report_descriptor_multiple_collections_test_application_define(void *first_unused_memory)
#endif
{
...
UINT                            descriptor_size = HID_REPORT_LENGTH;
    ...
    /* Inform user.  */
    printf("Running HID Report Descriptor Multiple Collections Test............. ");
...
UX_HOST_CLASS_HID_FIELD             *report_field;
    ...
    input_report_descriptor = report_get_id.ux_host_class_hid_report_get_report;

    /* Check the reports. */
    for(report_id = 0;; report_id++)
    {

        /* Check the fields */
        report_field = input_report_descriptor -> ux_host_class_hid_report_field;
        for(field = 0; report_field -> ux_host_class_hid_field_next_field != UX_NULL; field++)
        {

            if(
               report_field -> ux_host_class_hid_field_report_id != report_id ||
               report_field -> ux_host_class_hid_field_report_count != (8*report_id + 2*field + 1) ||
               report_field -> ux_host_class_hid_field_report_size !=  (8*report_id + 2*field + 2)
               )
            {
            ...
            report_field = report_field -> ux_host_class_hid_field_next_field;
        }

        if (input_report_descriptor -> ux_host_class_hid_report_next_report != UX_NULL)
        {

            /* Get the next input report descriptor. */
            status = ux_host_class_hid_report_id_get(hid, &report_get_id);
            if (status != UX_SUCCESS)
            {
