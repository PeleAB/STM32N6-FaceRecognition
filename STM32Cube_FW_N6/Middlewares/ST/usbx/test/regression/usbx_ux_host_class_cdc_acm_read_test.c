...
CHAR *                  memory_pointer;
    ...
    printf("Running ux_host_class_cdc_acm_read Test............................. ");
    stepinfo("\n");
...
ULONG                                               actual_length;
    ...
    /* Test read. */
    stepinfo(">>>>>>>>>>>>>>>> Test read semaphore error\n");

    endpoint = cdc_acm_host_data->ux_host_class_cdc_acm_bulk_in_endpoint;

    /* Delete semaphore so it generates error. */
    _ux_utility_semaphore_delete(&endpoint -> ux_endpoint_transfer_request.ux_transfer_request_semaphore);

    status = ux_host_class_cdc_acm_read(cdc_acm_host_data, buffer, 64, &actual_length);
    if (status == UX_SUCCESS)
    {
