...
#else
void    usbx_ux_host_class_hid_deactivate_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR *memory_pointer;
    ...
    /* Inform user.  */
    printf("Running ux_host_class_hid_deactivate Test........................... ");
...
static UINT  demo_class_hid_check(void)
{
...
UX_HCD     *hcd;
    ...
    } while(status == UX_HOST_CLASS_INSTANCE_UNKNOWN);
    if (status != UX_SUCCESS)
    {
    ...
    /**************************************************/
    /** Test case: if (hid -> ux_host_class_hid_client != UX_NULL) failure **/
    /** How: we do this by not registering any hid clients. **
    /**************************************************/
    ...
    while(demo_class_hid_get() == UX_SUCCESS)
        tx_thread_sleep(10);
