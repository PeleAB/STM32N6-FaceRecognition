...
#else
void    usbx_ux_host_class_storage_media_mount_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR *                          memory_pointer;
    ...
    /* Inform user.  */
    printf("Running ux_host_class_storage_media_mount Test...................... ");
    stepinfo("\n");
...
ULONG                                       rfree, cfree;
...
#if defined(UX_HOST_CLASS_STORAGE_NO_FILEX)
    /* Invoke not supported media functions for coverage.  */
    _ux_host_class_storage_media_mount(storage, 0);
    _ux_host_class_storage_media_open(storage, 0);
    _ux_host_class_storage_partition_read(storage, 0, 0);
#else
    stepinfo(">>>>>>>>>>>>>>> ux_host_class_storage_media_mount - unit_ready FAIL\n");
    ux_test_hcd_sim_host_set_actions(fail_bulk_out);
    status = _ux_host_class_storage_media_mount(storage, 0);
    if (status == UX_SUCCESS)
    {
        printf("ERROR #%d, code 0x%x\n", __LINE__, status);
        test_control_return(1);
    ...
    stepinfo(">>>>>>>>>>>>>>> ux_host_class_storage_media_mount - read FAIL\n");
    ux_test_hcd_sim_host_set_actions(fail_2nd_bulk_out);
    ram_disk_status = UX_SUCCESS;
    ram_disk_read_status = UX_ERROR;
    ram_disk_read_media_status = 0x003A02;
    status = _ux_host_class_storage_media_mount(storage, 0);
    if (status == UX_SUCCESS)
    {
        printf("ERROR #%d, code 0x%x\n", __LINE__, status);
    ...
    stepinfo(">>>>>>>>>>>>>>> ux_host_class_storage_media_mount - read FAIL SENSE ERROR\n");
    ram_disk_status = UX_SUCCESS;
    ram_disk_read_status = UX_ERROR;
    ram_disk_read_media_status = 0x023A00;
    status = _ux_host_class_storage_media_mount(storage, 0);
    if (status != UX_SUCCESS)
    {
        printf("ERROR #%d, code 0x%x\n", __LINE__, status);
