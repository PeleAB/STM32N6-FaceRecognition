...
#else
void    usbx_ux_device_class_storage_request_sense_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR *                          memory_pointer;
    ...
    /* Inform user.  */
    printf("Running ux_device_class_storage_request_sense Test.................. ");
    stepinfo("\n");
...
UX_DEVICE                                   *device;
    ...
    status = _test_transfer_data(demo_buffer, 64, UX_TRUE);
    if (status != UX_SUCCESS)
    {
        printf("ERROR #%d: code 0x%x\n", __LINE__, status);
        test_control_return(1);
