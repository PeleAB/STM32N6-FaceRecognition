...
#else
void    usbx_ux_device_class_storage_mode_sense_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR *                          memory_pointer;
    ...
    /* Inform user.  */
    printf("Running ux_device_class_storage_mode_sense Test..................... ");
    stepinfo("\n");
...
UX_SLAVE_CLASS_STORAGE                      *slave_storage;
    ...
    /************ UX_SLAVE_CLASS_STORAGE_MMC2_PAGE_CODE_CDROM ****************/
    ...
    status = _test_transfer_data(buffer, 128, UX_TRUE);
    if (status != UX_SUCCESS)
    {
        printf("ERROR #%d: code 0x%x\n", __LINE__, status);
        test_control_return(1);
    ...
    status = _test_transfer_data(buffer, 74, UX_TRUE);
    if (status != UX_SUCCESS)
    {
        printf("ERROR #%d: code 0x%x\n", __LINE__, status);
        test_control_return(1);
    ...
    status = _test_transfer_data(buffer, 24, UX_TRUE);
    if (status != UX_SUCCESS)
    {
        printf("ERROR #%d: code 0x%x\n", __LINE__, status);
        test_control_return(1);
    ...
    status = _test_transfer_data(buffer, 16, UX_TRUE);
    if (status != UX_SUCCESS)
    {
        printf("ERROR #%d: code 0x%x\n", __LINE__, status);
        test_control_return(1);
