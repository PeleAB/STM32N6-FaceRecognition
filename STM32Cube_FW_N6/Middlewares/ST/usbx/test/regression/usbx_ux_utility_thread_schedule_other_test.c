...
#else
void    usbx_ux_utility_thread_schedule_other_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR                            *cpool_start;
    ...
    /* Inform user.  */
    printf("Running ux_utility_thread_schedule_other Test....................... ");
...
ULONG  priority;
    ...
    status = _ux_utility_thread_schedule_other(UX_THREAD_PRIORITY_ENUM);
    if (status != UX_SUCCESS)
    {
    ...
    priority = _ux_system->ux_system_thread_lowest_priority;
    _ux_system->ux_system_thread_lowest_priority = TX_MAX_PRIORITIES;
    status = _ux_utility_thread_schedule_other(UX_THREAD_PRIORITY_ENUM);
    if (status == UX_SUCCESS)
    {
