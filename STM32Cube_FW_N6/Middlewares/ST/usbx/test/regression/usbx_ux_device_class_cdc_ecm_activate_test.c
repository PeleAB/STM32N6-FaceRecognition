...
#else
void usbx_ux_device_class_cdc_ecm_activate_test_application_define(void *first_unused_memory)
#endif
{

    /* Inform user.  */
    printf("Running ux_device_class_cdc_ecm_activate Test....................... ");
...
UCHAR                 tmp_byte;
    ...
    stepinfo(">>>>>>>>>>>>>>>>>>> Test modify ux_slave_class_cdc_ecm_instance_ activate/deactivate\n");
    UX_TEST_ASSERT(cdc_ecm_device != UX_NULL);
    cdc_ecm_device_bak = cdc_ecm_device;

    // ux_test_device_class_cdc_ecm_set_link_state(cdc_ecm_device, UX_DEVICE_CLASS_CDC_ECM_LINK_STATE_UP);
    cdc_ecm_device_bak -> ux_slave_class_cdc_ecm_parameter.ux_slave_class_cdc_ecm_instance_activate = UX_NULL;
    cdc_ecm_device_bak -> ux_slave_class_cdc_ecm_parameter.ux_slave_class_cdc_ecm_instance_deactivate = UX_NULL;
    ...
    stepinfo(">>>>>>>>>>>>>>>>>>> Test connect with control IF INT OUT endpoint\n");
    // printf("epAddr: %x\n", default_device_framework[INT_EP_DESC_POS + 2]);
    default_device_framework[INT_EP_DESC_POS + 2] = 0x03;
    ...
    stepinfo(">>>>>>>>>>>>>>>>>>> Test connect with control IF BULK IN endpoint\n");
    default_device_framework[INT_EP_DESC_POS + 2] = 0x83;
    default_device_framework[INT_EP_DESC_POS + 3] = 0x02;
    ux_test_dcd_sim_slave_connect(UX_FULL_SPEED_DEVICE);
    ux_test_hcd_sim_host_connect(UX_FULL_SPEED_DEVICE);
    UX_TEST_CHECK_SUCCESS(_wait_host_inst_change_to(100, 1));
    default_device_framework[INT_EP_DESC_POS + 3] = 0x03;
    ...
    stepinfo(">>>>>>>>>>>>>>>>>>> Test connect with data IF bulk endpoints swapped\n");
    // printf("epAddr: %x, %x\n", default_device_framework[BULK_EP_DESC_POS1 + 2], default_device_framework[BULK_EP_DESC_POS2 + 2]);
    tmp_byte = default_device_framework[BULK_EP_DESC_POS1 + 2];
    default_device_framework[BULK_EP_DESC_POS1 + 2] = default_device_framework[BULK_EP_DESC_POS2 + 2];
    default_device_framework[BULK_EP_DESC_POS2 + 2] = tmp_byte;
    ux_test_dcd_sim_slave_connect(UX_FULL_SPEED_DEVICE);
    ux_test_hcd_sim_host_connect(UX_FULL_SPEED_DEVICE);
    UX_TEST_CHECK_SUCCESS(_wait_host_inst_change_to(100, 1));
    ...
    /* Modify data interface for testing.  */
    framework_rm_data_if_alt();

    stepinfo(">>>>>>>>>>>>>>>>>>> Test connect with data IF BULK & BULK endpoint\n");
    ux_test_dcd_sim_slave_connect(UX_FULL_SPEED_DEVICE);
