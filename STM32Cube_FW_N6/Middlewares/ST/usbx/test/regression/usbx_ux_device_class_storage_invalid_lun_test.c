...
CHAR *                          memory_pointer;
    ...
    /* Inform user.  */
    printf("Running ux_device_class_storage_ Invalid LUN Test................... ");
    stepinfo("\n");
...
UX_SLAVE_CLASS_STORAGE                      *slave_storage;
    ...
    stepinfo(">>>>>>>>>>>>>>>> Test LUNs ~255\n");
    for (; storage -> ux_host_class_storage_lun < 255; storage -> ux_host_class_storage_lun ++)
    {
        status = test_host_class_storage_inquiry(storage, 0x00, buffer, 64);
        if (storage -> ux_host_class_storage_lun < global_storage_parameter.ux_slave_class_storage_parameter_number_lun)
        {
            if (status != UX_SUCCESS)
            {
    ...
    stepinfo(">>>>>>>>>>>>>>>> Test LUN 0\n");
    storage -> ux_host_class_storage_lun = 0;
    status = test_host_class_storage_inquiry(storage, 0x00, buffer, 64);
    if (status != UX_SUCCESS)
    {
