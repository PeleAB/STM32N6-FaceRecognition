...
#else
void    usbx_ux_utility_mutex_test_application_define(void *first_unused_memory)
#endif
{
...
const CHAR                       expect_error[] = {
    ...
    /* Inform user.  */
    printf("Running ux_utility_mutex_... Test................................... ");
...
UINT status;
    ...
    /* Mutex create. */
    status = ux_utility_mutex_create(&test_mutex, "test_mutex");
    if (status != UX_SUCCESS)
    {

        printf("ERROR #%d: mutex create fail\n", __LINE__);
        test_control_return(1);
    }

    /* Mutex on, expect no error. */
    error_callback_counter = 0;
    ux_utility_mutex_on(&test_mutex);
    if (error_callback_counter > 0)
    {

        printf("ERROR #%d: mutex on fail\n", __LINE__);
        error_counter ++;
    }

    /* Mutex off, expect no error. */
    error_callback_counter = 0;
    ux_utility_mutex_off(&test_mutex);
    if (error_callback_counter > 0)
    {

        printf("ERROR #%d: mutex off fail\n", __LINE__);
        error_counter ++;
    }

    /* Mutex create again, expect error. */
    status = ux_utility_mutex_create(&test_mutex, "test_mutex");
    if (status == UX_SUCCESS)
    {

        printf("ERROR #%d: mutex create must fail\n", __LINE__);
        test_control_return(1);
    }

    /* Mutex delete. */
    error_callback_counter = 0;
    ux_utility_mutex_delete(&test_mutex);
    if (error_callback_counter > 0)
    {

        printf("ERROR #%d: mutex delete fail\n", __LINE__);
        test_control_return(1);
    }

    /* Mutex on, expect error. */
    error_callback_counter = 0;
    ux_utility_mutex_on(&test_mutex);
    if (error_callback_counter == 0)
    {

        printf("ERROR #%d: expect mutex on error\n", __LINE__);
        error_counter ++;
    }
