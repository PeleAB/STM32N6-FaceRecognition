...
static UX_TRANSFER                     *dummy_transfer;
...
#define _CONFIGURATION_TOTAL_LENGTH (9+9+2*7)
...
static TX_SEMAPHORE        test_start_sem;
static TX_SEMAPHORE        test_end_sem;
...
#else
void usbx_ux_host_stack_transfer_request_abort_test_application_define(void *first_unused_memory)
#endif
{
...
UX_DEVICE_CLASS_DUMMY_PARAMETER  parameter;
    ...
    printf("Running ux_host_stack_transfer_request_abort Test................... ");

    /* Initialize the free memory pointer.  */
    stack_pointer = (CHAR *) first_unused_memory;
    memory_pointer = stack_pointer + (UX_DEMO_STACK_SIZE * 3);
    ...
    status =  tx_thread_create(&test_thread, "test thread", test_thread_entry, 0,
            stack_pointer + UX_DEMO_STACK_SIZE * 2, UX_DEMO_STACK_SIZE,
            20, 20, 1, TX_AUTO_START);
...
UCHAR               buffer[64];
    ...
    stepinfo(">>>> Dummy Class Transfer Abort Test\n");
    dummy_transfer = _ux_host_class_dummy_get_transfer_request(dummy, 0x82, 0);
    UX_TEST_ASSERT(dummy_transfer != UX_NULL);
    dummy_transfer -> ux_transfer_request_requested_length = 64;
    dummy_transfer -> ux_transfer_request_data_pointer = buffer;

    tx_semaphore_put(&test_start_sem);

    status = tx_semaphore_get(&test_end_sem, 2);
    UX_TEST_ASSERT(status != TX_SUCCESS);

    status = ux_host_stack_transfer_request_abort(dummy_transfer);
    UX_TEST_CHECK_SUCCESS(status);

    status = tx_semaphore_get(&test_end_sem, 100);
    UX_TEST_ASSERT(status == TX_SUCCESS);
...
UINT        status;
    ...
    tx_semaphore_create(&test_start_sem, "Test Start SEM", 0);
    tx_semaphore_create(&test_end_sem, "Test End SEM", 0);
    while(1)
    {
        tx_semaphore_get(&test_start_sem, TX_WAIT_FOREVER);
        status = ux_host_stack_transfer_request(dummy_transfer);
        UX_TEST_CHECK_SUCCESS(status);
        _ux_host_semaphore_get(&dummy_transfer->ux_transfer_request_semaphore, UX_WAIT_FOREVER);
        tx_semaphore_put(&test_end_sem);
    }
}
