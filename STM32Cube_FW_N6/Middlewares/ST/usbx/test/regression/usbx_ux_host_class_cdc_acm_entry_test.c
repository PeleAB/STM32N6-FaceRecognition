...
static unsigned char device_framework_full_speed[] = {
    ...
    */
    ...
    CFG_DESC(CFG_DESC_LEN + IAD_DESC_LEN + CDC_IFC_DESC_ALL_LEN, 2, 1)
    /* IAD 8 bytes */
    IAD_DESC(0)
    /* CDC_ACM interfaces */
...
#else
void usbx_ux_host_class_cdc_acm_entry_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR *                  memory_pointer;
    ...
    printf("Running ux_host_class_cdc_acm_entry Test............................ ");
...
UINT                                                ifc_off;
    ...
    for (test_n = 0; test_n < 2; test_n ++)
    {
        ...
        framework = test_n ? device_framework_high_speed : device_framework_full_speed;
        speed = test_n ? UX_HIGH_SPEED_DEVICE : UX_FULL_SPEED_DEVICE;
        iad_off = (18+9);
        ifc_off = test_n ? (18+10+9) : (18+9+8);

        if (test_n == 0)
        {

            stepinfo(">>>>>>>>>>>>>>>> Test %s IAD IAD.bClass error\n", test_n ? "no" : "with");

            /* Modify IAD.bClass.  */
            ux_test_dcd_sim_slave_disconnect();
            ux_test_hcd_sim_host_disconnect();
            ux_test_breakable_sleep(100, break_on_removal);
            framework[iad_off + 4] = 4;

            ux_test_dcd_sim_slave_connect(speed);
            ux_test_hcd_sim_host_connect(speed);
            ux_test_breakable_sleep(100, break_on_cdc_acm_all_ready);

            if (cdc_acm_host_control || cdc_acm_host_data)
            {

                printf("ERROR #%d.%d: expect enum fail\n", __LINE__, test_n);
                error_counter ++;
            }
            framework[iad_off + 4] = 2;

            stepinfo(">>>>>>>>>>>>>>>> Test %s IAD IAD.bSubClass error\n", test_n ? "no" : "width");

            /* Modify IAD.bSubClass.  */
            ux_test_dcd_sim_slave_disconnect();
            ux_test_hcd_sim_host_disconnect();
            ux_test_breakable_sleep(100, break_on_removal);
            framework[iad_off + 5] = 4;
            ...
            framework[iad_off + 5] = 2;

            stepinfo(">>>>>>>>>>>>>>>> Test %s IAD IAD.bClass&bSubClass 0&0\n", test_n ? "no" : "with");

            /* Modify IAD.bClass&bSubClass.  */
            ux_test_dcd_sim_slave_disconnect();
            ux_test_hcd_sim_host_disconnect();
            ux_test_breakable_sleep(100, break_on_removal);
            framework[iad_off + 4] = 0;
            framework[iad_off + 5] = 0;
            ...
            if (cdc_acm_host_control == UX_NULL)
            {
                ...
                printf("ERROR #%d.%d: expect enum OK\n", __LINE__, test_n);
                error_counter ++;
            }
            framework[iad_off + 4] = 2;
            framework[iad_off + 5] = 2;

            stepinfo(">>>>>>>>>>>>>>>> Test %s IAD IAD.bClass&bSubClass 0&4\n", test_n ? "no" : "with");
        ...
        stepinfo(">>>>>>>>>>>>>>>> Test %s IAD interface.bClass error\n", test_n ? "no" : "with");

        /* Modify Interface.bClass.  */
        ux_test_dcd_sim_slave_disconnect();
        ux_test_hcd_sim_host_disconnect();
        ux_test_breakable_sleep(100, break_on_removal);
        framework[ifc_off + 5] = 4;
        ...
        if (cdc_acm_host_control)
        {
            ...
            printf("ERROR #%d.%d: expect control interface fail\n", __LINE__, test_n);
            error_counter ++;
        }
        framework[ifc_off + 5] = 2;

        stepinfo(">>>>>>>>>>>>>>>> Test %s IAD interface.bSubClass error\n", test_n ? "no" : "with");

        /* Modify Interface.bSubClass.  */
        ux_test_dcd_sim_slave_disconnect();
        ux_test_hcd_sim_host_disconnect();
        ux_test_breakable_sleep(100, break_on_removal);
        framework[ifc_off + 6] = 4;
