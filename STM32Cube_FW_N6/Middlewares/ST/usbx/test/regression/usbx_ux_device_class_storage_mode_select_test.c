...
#else
void    usbx_ux_device_class_storage_mode_select_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR *                          memory_pointer;
    ...
    /* Inform user.  */
    printf("Running ux_device_class_storage_mode_select Test.................... ");
    stepinfo("\n");
...
static void _msc_mode_select_cases_test2(const char* __file__, int __line__)
{
UINT status;

    stepinfo("\n%s:%d:MSC MODE_SELECT tests\n", __file__, __line__);

    _test_init_cbw_MODE_SELECT(0);
    status = _test_send_cbw();
    if (status != UX_SUCCESS)
    {
        printf("ERROR #%d: code 0x%x\n", __LINE__, status);
...
UX_DEVICE                                   *device;
    ...
    _msc_mode_select_cases_test2(__FILE__, __LINE__);
