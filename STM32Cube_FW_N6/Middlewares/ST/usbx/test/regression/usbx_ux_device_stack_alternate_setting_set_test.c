...
#define HID_MOUSE_IFC1_DESC_ALL(ifc, interrupt_epa)     \
    ...
    0x09, 0x21, 0x10, 0x01, 0x21, 0x01, 0x22, LSB(HID_MOUSE_REPORT_LENGTH),\
    MSB(HID_MOUSE_REPORT_LENGTH),\
    /* Endpoint descriptor (Interrupt), wMaxPacketSize:0x08, n_trans:01 */\
    0x07, 0x05, (interrupt_epa), 0x03, 0x08, 0x08, 0x08,
...
#else
void    usbx_ux_device_stack_alternate_setting_set_test_application_define(void *first_unused_memory)
#endif
{
...
UX_SLAVE_CLASS_HID_PARAMETER    hid_parameter;
    ...
    /* Inform user.  */
    printf("Running ux_device_stack_alternate_setting_set Test.................. ");
...
ULONG                    endpoints_pool_number;
    ...
    /* UX_MEMORY_INSUFFICIENT:
        limit endpoint pool size so no free endpoint in pool to generate error.
     */
    endpoints_pool_number = device->ux_slave_device_endpoints_pool_number;
    device->ux_slave_device_endpoints_pool_number = 0;

    /* Invoke and check status. */
    status = _ux_device_stack_alternate_setting_set(1, 1);
    if (status != UX_MEMORY_INSUFFICIENT)
    {

        printf("ERROR #%d: Expect UX_MEMORY_INSUFFICIENT but get %x\n", __LINE__, status);
        error_counter ++;
    }
    /* Restore. */
    device->ux_slave_device_endpoints_pool_number = endpoints_pool_number;
#endif

    /* UX_DCD_CREATE_ENDPOINT fail:
        set DCD action to generate endpoint creation error.
     */
    ux_test_dcd_sim_slave_set_actions(dcd_endpoint_create_fail);

#if UX_TEST_MULTI_CLS_ON && UX_TEST_MULTI_IFC_ON
    ...
    if (status != UX_ERROR)
    {
        ...
        printf("ERROR #%d: Expect UX_ERROR but get %x\n", __LINE__, status);
        error_counter ++;
    }

    if (device->ux_slave_device_first_interface->ux_slave_interface_next_interface->ux_slave_interface_first_endpoint)
    {

        printf("ERROR #%d: Expect no endpoints\n", __LINE__);
        error_counter ++;
    }
    /* No action replacement. */
    ux_test_dcd_sim_slave_set_actions(UX_NULL);

    /* UX_NO_CLASS_MATCH:
        No class driver registered for the class.
     */
    class = _ux_system_slave -> ux_system_slave_interface_class_array[1];
    _ux_system_slave -> ux_system_slave_interface_class_array[1] = UX_NULL;
    ...
    if (status != UX_NO_CLASS_MATCH)
    {
        ...
        printf("ERROR #%d: Expect UX_NO_CLASS_MATCH but get %x\n", __LINE__, status);
        error_counter ++;
    }
    _ux_system_slave -> ux_system_slave_interface_class_array[1] = class;

    /* Select alternate setting 0. */
    status = _ux_device_stack_alternate_setting_set(1, 0);
    if (status != UX_SUCCESS)
    {

        printf("ERROR #%d: Expect success but get %x\n", __LINE__, status);
        error_counter ++;
    }

    ux_slave_class_status = class->ux_slave_class_status;
    ...
    class->ux_slave_class_status = ux_slave_class_status;

    /* case: device_framework_length == 0 */
    _ux_system_slave -> ux_system_slave_device_framework_length = 0;
    status = _ux_device_stack_alternate_setting_set(1, 0);
    if (status == UX_SUCCESS)
    {
    ...
    /* Normal case: switch alternate setting several times */
    status  = _ux_device_stack_alternate_setting_set(1, 0);
    status |= _ux_device_stack_alternate_setting_set(1, 1);
    status |= _ux_device_stack_alternate_setting_set(1, 0);
    status |= _ux_device_stack_alternate_setting_set(1, 1);
    ...
    /* Endpoint 0x83 should be available and ready to use */
    interface = class->ux_slave_class_interface;
    UX_TEST_ASSERT(interface != UX_NULL);

    endpoint = interface->ux_slave_interface_first_endpoint;
    UX_TEST_ASSERT(endpoint != UX_NULL);

    UX_TEST_ASSERT(endpoint->ux_slave_endpoint_descriptor.bEndpointAddress == 0x83);
    UX_TEST_ASSERT(endpoint->ux_slave_endpoint_state == 0x00);
#if UX_DEVICE_ENDPOINT_BUFFER_OWNER == 0
    UX_TEST_ASSERT(endpoint->ux_slave_endpoint_transfer_request.ux_slave_transfer_request_data_pointer != UX_NULL);
#else
    /* HID has no implement of alternate setting change, buffer not assigned.  */
    UX_TEST_ASSERT(endpoint->ux_slave_endpoint_transfer_request.ux_slave_transfer_request_data_pointer == UX_NULL);
#endif
#if !defined(UX_DEVICE_STANDALONE)
    UX_TEST_ASSERT(endpoint->ux_slave_endpoint_transfer_request.ux_slave_transfer_request_semaphore.tx_semaphore_id != 0);
    UX_TEST_ASSERT(endpoint->ux_slave_endpoint_transfer_request.ux_slave_transfer_request_semaphore.tx_semaphore_count == 0);
#endif
    UX_TEST_ASSERT(endpoint->ux_slave_endpoint_transfer_request.ux_slave_transfer_request_transfer_length == 16);

#endif /* #if UX_MAX_SLAVE_CLASS_DRIVER > 1 */
