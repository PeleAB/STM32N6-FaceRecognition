...
#else
void usbx_ux_device_stack_remote_wakeup_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR *                  memory_pointer;
    ...
    printf("Running ux_device_stack_ Remote Wakeup Test......................... ");
...
UX_TRANSFER                                        *transfer_request;
    ...
    stepinfo(">>>>>>>>>>>>>>>> GetDeviceStatus(), no error\n");
    ...
    if (status != UX_SUCCESS)
    {
        ...
        printf("ERROR #%d: GetDeviceStatus() code 0x%x\n", __LINE__, status);
        error_counter ++;
    }

    stepinfo(">>>>>>>>>>>>>>>> SetDeviceFeature(DEVICE_REMOTE_WAKEUP), error\n");
    ...
    if (status == UX_SUCCESS)
    {
        ...
        printf("ERROR #%d: SetDeviceFeature(DEVICE_REMOTE_WAKEUP) code 0x%x\n", __LINE__, status);
        error_counter ++;
    }

    stepinfo(">>>>>>>>>>>>>>>> ClearDeviceFeature(DEVICE_REMOTE_WAKEUP), error\n");
    ...
    if (status == UX_SUCCESS)
    {
        ...
        printf("ERROR #%d: ClearDeviceFeature(DEVICE_REMOTE_WAKEUP) code 0x%x\n", __LINE__, status);
        error_counter ++;
    }

    stepinfo(">>>>>>>>>>>>>>>> Test connect (HS)\n");
    ...
    stepinfo(">>>>>>>>>>>>>>>> SetDeviceFeature(DEVICE_REMOTE_WAKEUP), OK\n");
    ...
    stepinfo(">>>>>>>>>>>>>>>> GetDeviceStatus(), 0x02 - remote wakeup enabled\n");
    ...
    if ((buffer[0] & 0x02u) == 0)
    {
        ...
        printf("ERROR #%d: remote wakeup disabled\n", __LINE__);
        error_counter ++;
    }

    stepinfo(">>>>>>>>>>>>>>>> _ux_device_stack_host_wakeup(), UX_DCD_CHANGE_STATE called, OK\n");

    call_counter = 0;
    ux_test_dcd_sim_slave_set_actions(monitor_UX_DCD_CHANGE_STATE);
    status = _ux_device_stack_host_wakeup();
    if (call_counter == 0)
    {

        printf("ERROR #%d: UX_DCD_CHANGE_STATE not called\n", __LINE__);
        error_counter ++;
    }

    stepinfo(">>>>>>>>>>>>>>>> ClearDeviceFeature(DEVICE_REMOTE_WAKEUP), OK\n");
    ...
    stepinfo(">>>>>>>>>>>>>>>> GetDeviceStatus(), 0x02 - remote wakeup disabled\n");
    ...
    if ((buffer[0] & 0x02u) != 0)
    {
        ...
        printf("ERROR #%d: remote wakeup enabled\n", __LINE__);
        error_counter ++;
    }

    stepinfo(">>>>>>>>>>>>>>>> _ux_device_stack_host_wakeup(), UX_DCD_CHANGE_STATE not called, fail\n");
    ...
    if (call_counter > 0)
    {
        ...
        printf("ERROR #%d: UX_DCD_CHANGE_STATE called\n", __LINE__);
        error_counter ++;
    }
