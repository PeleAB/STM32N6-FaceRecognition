...
#else
void    usbx_ux_device_class_storage_read_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR *                          memory_pointer;
    ...
    /* Inform user.  */
    printf("Running ux_device_class_storage_read Test........................... ");
    stepinfo("\n");
...
UX_DEVICE                                   *device;
    ...
    stepinfo(">>>>>>>>>>>>>>> UX_SLAVE_CLASS_STORAGE_SCSI_READ16 - success\n");
    ...
    status = _test_transfer_data(buffer, 1024, UX_TRUE);
    if (status != UX_TRANSFER_STALLED)
    {
        printf("ERROR #%d: code 0x%x\n", __LINE__, status);
        test_control_return(1);
